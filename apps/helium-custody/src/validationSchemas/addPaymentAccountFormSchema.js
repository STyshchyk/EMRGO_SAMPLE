...
const createConditionalSchema = (condition, field) =>
  Yup.string().when(condition, {
    is: true,
    then: () =>
      Yup.string().nullable().max(50, `${field} is too long`).required(`${field} is required`),
...
const addPaymentAccountFormSchema = Yup.object().shape({
  ...
  accountNo: createCountryLabelSchema(
    "country.label",
    [USALabel, UKLabel],
  ...
  intermediaryBankBIC: createConditionalSchema("hasIntermediaryBank", "BIC"),
  intermediaryBankName: createConditionalSchema("hasIntermediaryBank", "Bank Name"),
  intermediaryBankAddress: createConditionalSchema("hasIntermediaryBank", "Address"),
  intermediaryBankCity: createConditionalSchema("hasIntermediaryBank", "City"),
  intermediaryBankPostCode: createConditionalSchema("hasIntermediaryBank", "Post Code"),
  intermediaryBankCountry: Yup.string().when("hasIntermediaryBank", {
    is: true,
    then: () => Yup.object().nullable().required("Country is required"),
    otherwise: () => Yup.object().nullable(),
  }),
  intermediaryBankAccountNo: createCountryLabelSchema(
    "intermediaryBankCountry.label",
    [USALabel, UKLabel],
    "Account number"
  ),
  intermediaryBankRouteCode: createCountryLabelSchema(
    "intermediaryBankCountry.label",
    [USALabel],
    "Routing number"
  ),
  intermediaryBankSortCode: createCountryLabelSchema(
    "intermediaryBankCountry.label",
    [UKLabel],
    "Sort code"
  ),
