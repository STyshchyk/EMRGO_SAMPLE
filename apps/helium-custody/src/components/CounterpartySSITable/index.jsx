...
const CounterpartySSITable = ({
  ...
  const dedupedEntities = Array.from(new Set(data.map((item) => item.entity)))
    .sort()
    .map((uniqueEntity) => {
      const matchedEntity = data.find((item) => item.entity === uniqueEntity);
      return {
  ...
  const entityOptionsList = dedupedEntities.map(({ entityName, entityId }) => ({
    label: entityName,
    value: entityName,
    id: entityId,
  }));

  const listBySelectedEntity = data.filter((item) => item?.entityId === entityFilterValue?.id);

  const dedupedCounterpartyNames = Array.from(
    new Set(listBySelectedEntity.map((item) => item.counterparty))
  ).sort();
