...
const useSecuritiesRegistrationDetailsContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useSecuritiesRegistrationDetailsContext hook must be called within a SecuritiesRegistrationDetailsContext.Provider component"
    );
  }
...
const convertSukukDataToArrayOfObjects = (sukukData) => {
  ...
  result.push([
    ...
    getNumberOfCertificates(sukukData?.issuanceAmount, sukukData?.denominationName?.value),
  ]);
  ...
  result.push([
    "Maturity Date",
    dateFormatter(sukukData?.maturityDate, "DD/MM/YYYY HH:MM") || "N/A",
  ]);
  ...
  result.push([
    "Relevant Courts / Jurisdiction for Arbitration",
    sukukData?.jurisdictionName?.name ?? "N/A",
  ]);
...
const OCI_WETHAQ_ASSETS_BUCKET_URL =
  "https://objectstorage.me-jeddah-1.oraclecloud.com/n/ax6tx9bvvc5i/b/wethaq-assets/o/";
...
const IssuanceTerms = () => {
  ...
  return (
    <Grid>
      ...
      <Typography paragraph>
        {t(
          "admission:Accordian.Issuance Terms.Capitalised terms used but not defined herein shall have the meanings given to them in the Issuer Terms"
        )}
        .
      </Typography>
      <Typography
      ...
      >
        {t(
          "admission:Accordian.Issuance Terms.The terms below have been confirmed by the Legal Counsel"
        )}
        .{" "}
        {t(
          "admission:Accordian.Issuance Terms.If you believe any of them are not accurate click to notify your Legal Counsel"
        )}
        .
      </Typography>
      <Grid container justifyContent="space-between">
        ...
        <Grid item>
          <CSVLink
            filename={`${admissionTermsheetData?.sukukData?.name}_termsheet_data.csv`}
            data={convertSukukDataToArrayOfObjects(admissionTermsheetData?.sukukData)}
            target="_blank"
          >
...
const SecuritiesAllocations = () => {
  ...
  return (
    <Grid>
      ...
      <SecurityAllocationPaymentView
        subscriptions={admissionTermsheetData.subscriptions || []}
        sukukData={admissionTermsheetData.sukukData || {}}
      />
    </Grid>
  );
};
...
const PrimaryIssuanceDetails = ({
  termsheetInfo,
  admissionTermsheetData,
  disableDocumentsAccordionItem,
}) => {
  ...
  const SECTION_DATA_OBJECT_ITEMS = [
    {
      ...
      accordionSummary: `${t(
        "securitiesRegistration:PrimaryIssuanceDetails.Sections.Registrar & Depository Documents.Title"
      )}`,
    ...
    },
    {
      ...
      accordionSummary: `${t(
        "securitiesRegistration:PrimaryIssuanceDetails.Sections.Issuance Terms.Title"
      )}`,
    ...
    },
    {
      ...
      accordionSummary: `${t(
        "securitiesRegistration:PrimaryIssuanceDetails.Sections.Issuer Details.Title"
      )}`,
    ...
    },
    {
      ...
      accordionSummary: `${t(
        "securitiesRegistration:PrimaryIssuanceDetails.Sections.Securities Allocations.Title"
      )}`,
