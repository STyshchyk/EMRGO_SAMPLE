...
function* uploadTFAVerificationFile({ payload }) {
  try {
    ...
    const errorMessage = extractErrorMessage(error);
    showToastErrorNotification(error, errorMessage);
    yield put(supportActionCreators.doUploadTFADocumentFailure({ message: errorMessage, key: payload.keyName || payload.name, index: payload.index }));
  }
}
...
const supportSaga = [
  ...
  takeLatest(supportActionTypes.UPLOAD_TFA_VERIFICATION_DOCUMENT_REQUESTED, uploadTFAVerificationFile),
  ...
  takeLatest(supportActionTypes.TFA_VERIFICATION_DOCUMENT_FETCH_REQUESTED, fetchTFAVerificationDocument),
];
