...
function* fetchExternalSecuritiesList({ payload }) {
  try {
    const response = yield call(wethaqAPIService.externalSecuritiesAPI.getExternalSecurities, payload);
    const { data } = response;
...
function* addExternalSecurties({ payload }) {
  try {
    ...
    const response = yield call(wethaqAPIService.externalSecuritiesAPI.addExternalSecurity, requestPayload);
    const { data } = response;
...
function* editExternalSecurties({ payload }) {
  const { requestPayload, successCallback } = payload;
  try {
    const response = yield call(wethaqAPIService.externalSecuritiesAPI.editExternalSecurity, requestPayload);
    const { data } = response;
...
function* deleteExternalSecurties({ payload }) {
  ...
  try {
    const response = yield call(wethaqAPIService.externalSecuritiesAPI.deleteExternalSecurity, requestPayload);
    const { data } = response;
...
function* addEquityExternalSecurity({ payload }) {
  try {
    ...
    const response = yield call(wethaqAPIService.externalSecuritiesAPI.addEquityExternalSecurity, requestPayload);
    const { data } = response;
...
function* queryExternalSecurities({ payload }) {
  try {
    const response = yield call(wethaqAPIService.externalSecuritiesAPI.searchExternalSecurities, payload);
    const { data } = response;
...
const externalSecuritiesSaga = [
  takeLatest(externalSecuritiesActionTypes.EXTERNAL_SECURITIES_LIST_FETCH_REQUESTED, fetchExternalSecuritiesList),
  takeLatest(externalSecuritiesActionTypes.EXTERNAL_SECURITIES_ADD_REQUESTED, addExternalSecurties),
  takeLatest(externalSecuritiesActionTypes.EXTERNAL_SECURITIES_EDIT_REQUESTED, editExternalSecurties),
  takeLatest(externalSecuritiesActionTypes.EXTERNAL_SECURITIES_DELETE_REQUESTED, deleteExternalSecurties),
  takeLatest(externalSecuritiesActionTypes.EXTERNAL_SECURITIES_ADD_EQUITY_REQUESTED, addEquityExternalSecurity),
  takeLatest(externalSecuritiesActionTypes.EXTERNAL_SECURITIES_SEARCH_REQUESTED, queryExternalSecurities),
];
