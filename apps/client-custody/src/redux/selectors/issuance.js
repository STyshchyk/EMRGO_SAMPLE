...
export const selectIssuanceSize = createSelector([selectSubscriptionData], (subscriptionData) => parseFloat(subscriptionData?.issuanceAmount ?? 0));

export const selectProfitRate = createSelector([selectSubscriptionData], (subscriptionData) => parseFloat(subscriptionData?.profitRate ?? 0));

export const selectDenominationValue = createSelector([selectSubscriptionData], (subscriptionData) => parseFloat(subscriptionData?.denominationName?.value ?? 0));

export const selectCurrencyName = createSelector([selectSubscriptionData], (subscriptionData) => subscriptionData?.currencyName?.name ?? 'USD');

export const selectSubscriptions = createSelector([selectSubscriptionData, selectEnabledInvestors, selectDenominationValue], (subscriptionData, enabledInvestors, denominationValue) => {
  const result = [];
  ...
  if (subscriptionData?.subscriptions.length > 0 && denominationValue > 0) {
    ...
    subscriptions.forEach((subscription) => {
      ...
      result.push({
        ...
        eoiStatus: changeCase.titleCase(status),
      });
    });
  }
  return result;
...
export const selectAllClosingSignoffData = createSelector([selectClosingData], (closingData) => {
  ...
  const filteredClosingData = [arrangerSignoffData, coArrangerSignoffData, obligorSignoffData, issuerSignoffData].filter((i) => i.entityGroupID);

  return filteredClosingData;
});

export const selectOwnClosingSignoffData = createSelector([selectAllClosingSignoffData, selectCurrentEntityGroup], (allClosingSignoffs, currentEntityGroup) => {
  ...
  if (termsheet) {
    return {
      ...produce(termsheet, (draft) => {
        ...
        draft.currency = termsheet.currencyName ? { label: termsheet.currencyName.name, value: termsheet.currencyName.id } : intialTermsheetValues.currencyName;
        draft.denomination = termsheet.denominationName ? { label: termsheet.denominationName.value, value: termsheet.denominationName.id } : intialTermsheetValues.denominationName;
        draft.profitRateTerms = termsheet.profitRateTermsName ? { label: termsheet.profitRateTermsName.name, value: termsheet.profitRateTermsName.id } : intialTermsheetValues.profitRateTermsName;
        draft.frequency = termsheet.frequencyName ? { label: termsheet.frequencyName.name, value: termsheet.frequencyName.id } : intialTermsheetValues.frequencyName;
        ...
        draft.formOfOffering = termsheet.formOfOfferingName ? { label: termsheet.formOfOfferingName.name, value: termsheet.formOfOfferingName.id } : intialTermsheetValues.formOfOfferingName;
        draft.useOfProceeds = termsheet.useOfProceedsName ? { label: termsheet.useOfProceedsName.name, value: termsheet.useOfProceedsName.id } : intialTermsheetValues.useOfProceedsName;
        ...
        draft.ranking = termsheet.rankingName ? { label: termsheet.rankingName.name, value: termsheet.rankingName.id } : intialTermsheetValues.rankingName;
        draft.listing = termsheet.listingName ? { label: termsheet.listingName.name, value: termsheet.listingName.id } : intialTermsheetValues.listingName;
        draft.rating = termsheet.ratingName ? { label: termsheet.ratingName.name, value: termsheet.ratingName.id } : intialTermsheetValues.ratingName;
        draft.governingLaw = termsheet.governingLawName ? { label: termsheet.governingLawName.name, value: termsheet.governingLawName.id } : intialTermsheetValues.governingLawName;
        draft.jurisdiction = termsheet.jurisdictionName ? { label: termsheet.jurisdictionName.name, value: termsheet.jurisdictionName.id } : intialTermsheetValues.jurisdictionName;
      }),
    };
  }
...
export const selectFetchingEngagementRequests = (state) => state.issuance.isFetchingEnagementRequests;
export const selectAreEngagementDocumentsUploading = (state) => state.issuance.areEngagementDocumentsUploading;
...
export const selectFetchingAdmissionTermsheetData = (state) => state.issuance.isFetchingAdmissionTermsheetData;
...
export const selectEngagementObligorProposalSignedDocURL = (state) => state.issuance?.engagementObligorProposalSignedDocURL?.link;
export const selectFetchingEngagementObligorProposalSignedDocURL = (state) => state.issuance?.isFetchingEngagementObligorProposalSignedDocURL;
export const selectIsSubmittingAdmissionTerms = (state) => state.issuance.isSubmittingAdmissionTerms;
...
export const selectTermsheetInfo = createSelector([selectTermsheet], (termsheet) => ({
  ...
  certificateCount: convertNumberToIntlFormat(parseInt(termsheet?.issuanceAmount, 10) / parseInt(termsheet?.denominationName?.value, 10)),
...
export const selectTermsheetInfoAr = createSelector([selectTermsheet], (termsheet) => ({
  ...
  arranger: termsheet?.arranger,
  arrangerDetails: termsheet?.arrangerDetails ?? null,
  certificateCount: parseInt(termsheet?.issuanceAmount, 10) / parseInt(termsheet?.denominationName?.value, 10),
...
export const selectPrivatePlacementIssuanceStatusObject = createSelector([selectIssuanceOverview], (issuanceOverview) => {
  const STATES_ENUM = {
  ...
  if (!issuanceOverview?.status || issuanceOverview?.distributionMethodName?.name !== 'Private Placement') return stages;
...
export const selectCapitalMarketAuthorityData = (state) => state.issuance.capitalMarketAuthorityData;

export const selectCapitalMarketAuthorityDocumentURLs = (state) => state.issuance.capitalMarketAuthorityDocumentURLs;
...
export const selectHasSubmittedBypassSignedEngagement = (state) => state.issuance.hasSubmittedBypassSignedEnagagement;

export const selectIsSubmittingBypassSignedEngagement = (state) => state.issuance.isSubmittingBypassSignedEngagement;

export const selectHasEngagementLetterUploaded = (state) => state.issuance.hasEngagementLetterUploaded;

export const selectIssuanceOverviewProviders = (state) => state.issuance?.issuanceOverview?.providers;
