...
export const selectCounterpartyList = createSelector([selectCounterpartyData], (counterparties) => {
  if (counterparties?.length > 0) {
    return counterparties?.map((item) => ({
      id: item.id,
      entityId: item?.entity ? { label: item.entity?.corporateEntityName, value: item.entity?.id } : null,
...
export const selectCounterpartySSIList = createSelector([selectCounterpartySSIData], (counterpartiesSSI) => {
  if (counterpartiesSSI?.length > 0) {
    return counterpartiesSSI?.map((item) => ({
      id: item.id,
      entityId: item?.entity ? { label: item.entity?.corporateEntityName, value: item.entity?.id } : null,
      counterpartyId: item?.counterparty ? { label: item.counterparty?.counterpartyId, value: item.counterparty?.id } : null,
      longName: item?.counterparty ? { label: item?.counterparty?.longName, value: item?.counterparty?.longName } : null,
      ssiLabel: item.ssiLabel,
      settlementLocationId: item?.settlementLocation ? { label: item.settlementLocation?.name, value: item.settlementLocation?.id } : null,
      deliveryAgentIdentifierTypeId: item?.deliveryAgentIdentifierType ? { label: item.deliveryAgentIdentifierType?.name, value: item.deliveryAgentIdentifierType?.id } : null,
      deliveryAgentIdentifier: item?.deliveryAgentIdentifier,
      sellerIdentifierTypeId: item?.sellerIdentifierType ? { label: item.sellerIdentifierType?.name, value: item.sellerIdentifierType?.id } : null,
