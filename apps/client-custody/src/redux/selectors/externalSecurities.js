...
export const selectExternalSecuritiesData = (state) => state.externalSecurities.externalSecuritiesData;
...
export const selectExternalSecuritiesList = createSelector([selectExternalSecuritiesData], (externalSecuritiesData) => {
  if (Array.isArray(externalSecuritiesData) && externalSecuritiesData?.length > 0) {
    return externalSecuritiesData?.map((item) => ({
      ...
      currencyName: item?.currencyName ? { label: item.currencyName.name, value: item.currencyName.id } : null,
      denomination: item.denomination,
      denominationName: item?.denominationName ? { label: item.denominationName.value, value: item.denominationName.id } : null,
      frequency: item.frequency,
      frequencyName: item?.frequencyName ? { label: item.frequencyName.name, value: item.frequencyName.id } : null,
      id: item.id,
      isin: item.isin,
...
export const selectAllExternalSecurities = createSelector([selectExternalSecuritiesData], (externalSecuritiesList) => {
...
export const selectExternalSecuritySearchResultsRaw = (state) => state.externalSecurities.listOfExternalSecuritySearchResults;
export const selectExternalSecuritySearchResults = createSelector([selectExternalSecuritySearchResultsRaw], (externalSecuritySearchResults) => {
