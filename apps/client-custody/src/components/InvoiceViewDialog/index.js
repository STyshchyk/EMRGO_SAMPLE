...
import { dateRenderer, roundNumber, idRenderer } from '../../constants/renderers';
...
const InvoiceViewDialog = ({ open, handleClose, selectedRow, modalType, fetchInvoiceParams }) => {
  ...
  const remoteSafekeepingCharges = useMemo(() => invoiceConfig?.safe_keeping_item || [], [invoiceConfig?.safe_keeping_item]);
  const remoteTrxCharges = useMemo(() => invoiceConfig?.transaction_items || [], [invoiceConfig?.transaction_items]);
  const remoteOutOfPocketExpenses = useMemo(() => invoiceConfig?.pocket_expenses || [], [invoiceConfig?.pocket_expenses]);
  ...
  }, [remotePaymentHolidays, remoteSafekeepingCharges, remoteTrxCharges, remoteOutOfPocketExpenses]);
  ...
  const handleInvoiceSubmit = (values) => {
    ...
    dispatch(billingActionCreators.doUpdateInvoiceRequest({ requestPayload, invoiceId: selectedRow?.id, fetchInvoiceParams }));
  };
  ...
  const handleApprove = () => {
    ...
    dispatch(billingActionCreators.doApproveInvoiceRequest({ requestPayload, invoiceId: selectedRow?.id, fetchInvoiceParams }));
    handleClose();
  };

  return (
    <Fragment>
      <Dialog fullWidth open={open} onClose={handleClose} aria-labelledby="form-dialog-title" scroll="body" maxWidth="lg">
        <Formik
          initialValues={{
            ...
            invoiceAmount: roundNumber(Number(totalInvoiceAmount === null ? selectedRow?.amount : totalInvoiceAmount), BANK_AMOUNT_DP) || 0,
          }}
        ...
        >
          {({ handleSubmit, values, setFieldValue }) => {
            const handleInvoiceCalculation = () => {
              ...
              console.log('🚀 ~ file: index.js:249 ~ handleInvoiceCalculation ~ requestPayload:', JSON.stringify(requestPayload));

              dispatch(billingActionCreators.doCalculateInvoiceRequest({ requestPayload, invoiceId: selectedRow?.id }));
              setFieldValue('invoiceAmount', roundNumber(Number(totalInvoiceAmount === null ? selectedRow?.amount : totalInvoiceAmount), BANK_AMOUNT_DP) || 0);
            };

            return (
              <form onSubmit={handleSubmit}>
                <DialogTitle id="form-dialog-title"> {t('Invoice Management.View Dialog.Invoice', { type: changeCase.titleCase(modalType) })}</DialogTitle>
                ...
                <DialogContent>
                  <Box mb={2}>
                    <Grid container spacing={2}>
                      <Grid item xs={12} md={12} lg={6} container>
                        <Grid item xs={6} container alignContent="flex-start">
                          <Typography className="mt-4">{t('Invoice Management.View Dialog.Client')}</Typography>
                        </Grid>
                      ...
                      <Grid item xs={12} md={12} lg={6} container>
                        <Grid item xs={6} container alignContent="flex-start">
                          <Typography className="mt-4">{t('Invoice Management.View Dialog.Invoice Reference')}</Typography>
                        </Grid>
                        <Grid item xs={6} container alignContent="center" className="px-1">
                          <Field fullWidth disabled component={TextField} name="invoiceRef" variant="filled" />
                        </Grid>
                      </Grid>
                      <Grid item xs={12} md={12} lg={6} container>
                        <Grid item xs={6} container alignContent="flex-start">
                          <Typography className="mt-4">{t('Invoice Management.View Dialog.Invoice Date')}</Typography>
                        </Grid>
                        <Grid item xs={6} container alignContent="center" className="px-1">
                          <Field fullWidth disabled component={TextField} name="invoiceDate" variant="filled" />
                        </Grid>
                      </Grid>
                      <Grid item xs={12} md={12} lg={6} container>
                        <Grid item xs={6} container alignContent="flex-start">
                          <Typography className="mt-4">{t('Invoice Management.View Dialog.Minimum Charge (USD)')}</Typography>
                        </Grid>
                        <Grid item xs={6} container alignContent="center" className="px-1">
                    ...
                    <PaymentHolidaysSection modalType={modalType} selectedRow={selectedRow} localPaymentHolidays={localPaymentHolidays} setLocalPaymentHolidays={setLocalPaymentHolidays} />

                    <OutOfPocketExpensesSections modalType={modalType} localOutOfPocketExpenses={localOutOfPocketExpenses} setLocalOutOfPocketExpenses={setLocalOutOfPocketExpenses} />

                    <Grid container justifyContent="flex-end" alignContent="center">
                      <Grid xs item container direction="column" justifyContent="center" alignContent="flex-end">
                        <Typography>Invoice Amount (USD)</Typography>
                      </Grid>

                      <Field disabled={modalType !== 'amend'} component={TextField} name="invoiceAmount" variant="filled" className="w-[20rem] ml-6 mr-2 " />
                      {modalType === 'amend' && (
                        <Tooltip title="Update" placement="top">
                          <Fab aria-label="update" size="small" color="primary" onClick={() => handleInvoiceCalculation()}>
                            <RefreshIcon />
                          </Fab>
                        </Tooltip>
                      )}
