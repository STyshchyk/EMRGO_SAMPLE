...
const TransactionChargesSection = ({
  modalType,
  selectedRow,
    ...
    <Box my={4}>
      <MaterialTable
        title={
          <Typography
            variant="h6"
            className={transactionChargeOverrides.length > 0 ? "text-yellow-500" : ""}
          >
            Transaction Charges
                  ...
                  new Promise((resolve, reject) => {
                    const schema = yup.object().shape({
                      ...
                      calculated_txn: yup
                        ...
                        .positive("Number of transactions must be a positive number"),
                      total_amount: yup
                        .number()
                ...
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    ...
                    const requestObject = {
                      ...
                      client_rate_card_id: selectedRow?.id,
                      transaction_type_id: newData?.transaction_type_id?.value
                        ? newData?.transaction_type_id?.value
                        : newData?.transaction_type_id,
                      asset_type_id: newData?.asset_type_id?.value
      ...
      {transactionChargeOverrides.length > 0 && showTransactionChargeOverridesError ? (
        <Fragment>
          ...
          >
            <AlertTitle>
              {t("Invoice Management.View Dialog.TransactionChargeOverrideMessageKey", {
                count: transactionChargeOverrides.length,
              })}
            </AlertTitle>
            <ul>
              {transactionChargeOverrides.map((override) => (
                <li className="list-disc" key={override.id}>
                  <Typography>{`Total amount for the ${override.asset_type_name} ${
                    override.transaction_type_name
                  } in ${override.asset_country_icsd_name} should be $${currencyRenderer(
