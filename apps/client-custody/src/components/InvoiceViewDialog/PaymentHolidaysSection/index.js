...
import { idRenderer, dateRenderer } from '../../../constants/renderers';
...
const PaymentHolidaysSection = ({ modalType, selectedRow, localPaymentHolidays, setLocalPaymentHolidays }) => {
  ...
  const handleToggleApplyHoliday = (requestObject) => {
    ...
    tempLocalPaymentHolidays[foundIndex] = { ...foundPaymentHolidays, isApplied: !foundPaymentHolidays.isApplied };
  ...
  const holidayTableColumns = [
    ...
    },
    {
      ...
      initialEditValue: '',
      editComponent: (props) => <MuiTextField fullWidth label="Narrative" variant="filled" value={props.value} onChange={(e) => props.onChange(e.target.value)} />,
    },
    {
      title: 'Apply',
      field: 'isApplied',
      ...
      editable: 'never',
      // align: 'right',
      render: (rowData) => <Checkbox color="primary" checked={rowData.isApplied} onChange={() => handleToggleApplyHoliday(rowData)} inputProps={{ 'aria-label': 'primary checkbox' }} />,
    },
  ];
  ...
  const handleDeletePaymentHoliday = (requestObject, resolve) => {
    const filteredCharges = localPaymentHolidays.filter((holiday) => !requestObject.includes(holiday.id));
    ...
    <Box my={4}>
      ...
      {paymentHolidayOverlaps.length > 0 && showPaymentHolidayOverlapsError ? (
        <Fragment>
          ...
          >
            <AlertTitle>{t('Invoice Management.View Dialog.OverlappingPeriodMessageKey', { count: paymentHolidayOverlaps.length })}</AlertTitle>
            <ul>
