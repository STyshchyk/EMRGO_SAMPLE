...
const SafekeepingChargesSection = ({ modalType, selectedRow, securityTypeOptions, countriesLookup, localSafekeepingCharges, setLocalSafekeepingCharges }) => {
  ...
  const safekeepingTableColumns = [
    ...
    },
    {
      ...
      field: 'band_start',
      editComponent: (props) => <MuiTextField fullWidth label="Start ($)" variant="filled" value={props.value} onChange={(e) => props.onChange(e.target.value)} />,
    },
    {
      title: 'Txn Band End',
      field: 'band_end',
      editComponent: (props) => <MuiTextField fullWidth label="End ($)" variant="filled" value={props.value} onChange={(e) => props.onChange(e.target.value)} />,
    },
    {
      title: 'Charge(BPS)',
      field: 'charge_in_bps',
    ...
    },
    {
      ...
      initialEditValue: '',
      editComponent: (props) => <MuiTextField fullWidth label="Valuation" variant="filled" value={props.value} onChange={(e) => props.onChange(e.target.value)} />,
    },
    {
      title: 'Charge(USD)',
      field: 'charge',
  ...
  const processedSafeKeepingData = localSafekeepingCharges.map((band) => {
    const concatedKey = `${changeCase.camelCase(band.asset_country_icsd_name)}_${changeCase.camelCase(band.asset_type_name)}`;
                ...
                onRowAdd: (newData) =>
                  new Promise((resolve, reject) => {
                    const schema = yup.object().shape({
                      ...
                      charge: yup.number().required('Total amount is required').positive('Total must be a positive number'),
                    });
