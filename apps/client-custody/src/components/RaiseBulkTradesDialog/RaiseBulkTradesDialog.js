...
const RaiseBulkTradesDialog = ({ open, handleClose, tableData }) => {
  ...
  const raiseBulkSettlementInstructions = (payload) => dispatch(paymentAndSettlementActionCreators.doRaiseBulkSettlementInstructions(payload));
  const fetchPaymentsList = () => dispatch(paymentAndSettlementActionCreators.doFetchPaymentsList());

  const getCounterpartyById = (counterparty, counterpartySSI) => {
    const selectedCounterparty = counterpartiesList.filter((ctpy) => ctpy.counterpartyId === counterparty)[0];
    const selectedSSI = selectedCounterparty?.counterpartySSI?.filter((ssi) => ssi.ssiLabel === counterpartySSI)[0];
  ...
  const getExternalSecurityId = (inputtedIsin) => {
    const selectedExtSecurity = externalSecuritiesList.filter(({ isin }) => isin === inputtedIsin)[0];
  ...
  const getSettlementId = (settlementType) => {
    const selectedSettlementType = dropdownDataOptions?.settlementInstructionType?.filter((d) => d?.name === settlementType)[0];
  ...
  const handleImportCSVData = () => {
    ...
    const instructions = importedData.map((entry) => {
      const { settlementType, isin, counterparty, counterpartySSI, quantity, price, principalAmount, accruedInterest, settlementAmount, internalTradeRef } = entry;
      ...
      return {
        ...
        principalAmount: !isFreeOfPayment ? parseFloat(principalAmount.replace(',', '')) : undefined,
        accruedInterest: !isFreeOfPayment ? parseFloat(accruedInterest.replace(',', '')) : undefined,
        settlementAmount: !isFreeOfPayment ? parseFloat(settlementAmount.replace(',', '')) : undefined,
