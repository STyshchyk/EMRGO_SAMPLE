...
const SelectAllButton = ({ onChange, label, checked }) => (
  <div className={style.buttonCheckbox}>
    <UtilCheckbox name="investors" onClick={() => onChange(!checked)} label={label} checked={checked} />
  </div>
);
      ...
      rows.push(
        <div className={style.tableRow} key={entry.id}>
          <div className={style.rowData}>
            ...
            <div className={style.tableCol}>
              <p className={style.tableData}>{entry.user ? `${entry.user.firstName || ''} ${entry.user.middleName || ''} ${entry.user.lastName || ''}` : ''}</p>
            </div>
          </div>
        </div>,
      );
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className={style.issuanceDataTable}>
            <div className={style.tableHeader}>
              <div className={cx(style.tableCol, locale.rtl ? style.tableCol__right : style.tableCol__left)}>
          ...
          <SelectAllButton onChange={(select) => handleSelectAllToggle(select, setValues)} label={selectAll ? t('investors:Unselect All') : t('investors:Select All')} checked={selectAll} />
          <input type="submit" value={t('investors:Invite Investors')} className={cx('btn btn--primary btn--thin', style.block)} />
        </form>
      )}
    </Formik>
  );
