...
const ReviewSecurityDialog = ({ data, open, handleClose }) => {
  ...
  const handleClickDownloadCSVButton = () => {
    ...
    const csvBuilder = new CsvBuilder(csvFileName).setColumns(listOfColumnNames).addRows([listOfRowValues]);
  ...
  return (
    <Fragment>
      <Dialog fullWidth maxWidth="md" open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
        <StyledDialogHeader title={t('agency_services:Security Information')} handleClose={handleClose} />
        <DialogContent>
        ...
        <DialogActions>
          <Button variant="contained" startIcon={<CloudDownloadIcon />} onClick={handleClickDownloadCSVButton} color="primary">
            <strong>CSV</strong>
          </Button>
          <Button variant="contained" startIcon={<CloudDownloadIcon />} onClick={handleClickDownloadPDFButton} color="primary">
            <strong>PDF</strong>
          </Button>
        </DialogActions>
      </Dialog>
