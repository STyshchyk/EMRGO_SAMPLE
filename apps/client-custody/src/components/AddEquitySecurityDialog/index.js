...
const AddEquitySecurityDialog = ({ open, handleClose, selectedRow, setSelectedRow }) => {
  ...
  const [selectedSecurityIdentifierTypeOption, setSelectedSecurityIdentifierTypeOption] = useState(null);
  ...
  console.debug('DEBUG selectedRow attributes: ', JSON.stringify(selectedRow?.attributes, null, 2));
  ...
  console.debug('DEBUG currency: ', getCurrencyOption('48f00a2b-6415-44d6-8904-53a0a9ea6b1a'));
  console.debug('DEBUG selectedRow: ', selectedRow);
      ...
      <MuiPickersUtilsProvider utils={MomentUtils} locale={theme.locale.altLocale}>
        ...
        >
          {({ handleSubmit, setFieldValue, values }) => {
            const listOfUserSelectedSecurityIdentifierTypes = values.attributes.map(({ securityIdTypeName }) => securityIdTypeName);
            ...
            return (
              <form onSubmit={handleSubmit} noValidate>
                <DialogTitle id="form-dialog-title">
                  <Grid container justifyContent="space-between">
                    <Grid item xs container alignContent="center">
                      <strong>{isEdit ? t('external_securities:External Securities.Edit Equity Security') : t('external_securities:External Securities.New Equity Security')}</strong>
                    </Grid>
                      ...
                      <Grid container className="mt-4">
                        ...
                        <Grid item xs={12} md={6} lg={6} className="px-1">
                          <Field
                            fullWidth
                            component={TextField}
                            label={t('external_securities:External Securities.Add Security Form.Security Long Name')}
                            name="longName"
                            variant="filled"
                            type="text"
                          />
                          ...
                          <Fragment>
                            <Grid container item className="mt-4">
                              <Grid item xs={12} md={6}>
                                <Typography className="mt-4">{'Add Security Identifier Type ***'}</Typography>
                              </Grid>
                                ...
                                <Grid item xs={11} md={11}>
                                  <FormControl className="w-full">
                                    <Select
                                      ...
                                      styles={selectStyles}
                                      menuPortalTarget={document.body}
                                      options={getDropdownValues(formOptions?.securityIdentifier).filter(
                                        ({ label }) => !['Long Name', 'Short Name'].includes(label) && !listOfUserSelectedSecurityIdentifierTypes.includes(label),
                                      )}
                                      onChange={(newValue) => {
                                  ...
                                  <Typography variant="caption" color="error" className="ml-4">
                                    {values.attributes.length === 0 && 'At least one security identifier is required'}
                                  </Typography>
                                </Grid>
                                <Grid item xs={1} md={1}>
                                  <IconButton
                                    ...
                                    onClick={() => {
                                      ...
                                      push({ securityIdTypeName: selectedSecurityIdentifierTypeOption?.label, identifierId: selectedSecurityIdentifierTypeOption?.value, value: '' });
                                    }}
                                    size="large">
                                    <AddIcon />
                                  </IconButton>
                                  ...
                                  <Grid container className="mt-4">
                                    ...
                                    <Grid item container xs={12} md={6}>
                                      <Grid item xs={11} md={11} className="px-1">
                                        <Field fullWidth component={TextField} label={item.securityIdTypeName} name={`attributes.${index}.value`} variant="filled" type="text" />
                                      </Grid>
                                      <Grid item xs={1} md={1}>
                                        <IconButton
                                          color="secondary"
                ...
                <DialogActions>
                  ...
                  <Grid item>
                    <Button fullWidth type="submit" variant="contained" color="primary" disabled={values.attributes.length === 0}>
