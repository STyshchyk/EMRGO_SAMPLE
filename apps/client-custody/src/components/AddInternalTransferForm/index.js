...
export const generateEntityOptionsList = (data) => data.map((entity) => ({ value: entity.id, label: entity.corporateEntityName }));
export const generateWethaqAccountOptionsList = (data) =>
  data.map((account) => ({
    value: { accountId: account.id, entityId: account.group.entity?.id, currencyId: account.currency?.id, currency: account.currency?.name, accountBalance: account.accountBalance },
    ...
    <Formik initialValues={initialValues} onSubmit={handleSubmit} enableReinitialize>
      {({ values, setFieldValue, isSubmitting }) => {
        const sourceEntityChange = (selectedEntity) => {
          ...
          sourceAccountsDropdown = generateWethaqAccountOptionsList(sourceAccounts.filter((account) => (selectedEntity ? account.group.entity.id === selectedEntity.value : true)));
        };

        const sourceAccountChange = (selectedAccount) => {
          setFieldValue('sourceAccount', selectedAccount);
          const tempEntitiesList = generateEntityOptionsList(sourceOwners.filter((entity) => (selectedAccount ? entity.id === selectedAccount.value.entityId : true)));
          ...
          if (selectedAccount) {
            destinationAccountsDropdown = generateWethaqAccountOptionsList(
              destinationAccounts
                .filter((account) => (values.destinationEntity ? account.group.entity.id === values.destinationEntity.value : true))
                .filter((account) => (selectedAccount ? account.currencyId === selectedAccount.value.currencyId : true))
                .filter((account) => (selectedAccount ? account.id !== selectedAccount.value.accountId : false)),
            );
        ...
        const destinationEntityChange = (selectedEntity) => {
          ...
          destinationAccountsDropdown = generateWethaqAccountOptionsList(
            destinationAccounts
              ...
              .filter((account) => (values.sourceAccount ? account.currencyId === values.sourceAccount.value.currencyId : true)),
          );
        };

        const destinationAccountChange = (selectedAccount) => {
          setFieldValue('destinationAccount', selectedAccount);
          const tempEntities = generateEntityOptionsList(destinationOwners.filter((entity) => (selectedAccount ? entity.id === selectedAccount.value.entityId : true)));

          if (selectedAccount) {
            sourceAccountsDropdown = generateWethaqAccountOptionsList(
              sourceAccounts
                .filter((account) => (values.sourceEntity ? account.group.entity.id === values.sourceEntity.value : true))
                .filter((account) => (selectedAccount ? account.currencyId === selectedAccount.value.currencyId : true))
                .filter((account) => (selectedAccount ? account.id !== selectedAccount.value.accountId : false)),
            );
        ...
        const showSelectedCurrency = () => {
          ...
          if (values.sourceAccount || values.destinationAccount) {
            currency = values.sourceAccount?.value.currency || values.destinationAccount?.value.currency;
          }
          return currency;
        };
            ...
            <Grid container spacing={2}>
              ...
              <InlineFormField label={t('Internal Transfer.Transfer Amount')}>
                <Field
                  ...
                  InputProps={{
                    inputComponent: CustomNumberInputField,
                    endAdornment: <InputAdornment position="end">{showSelectedCurrency()}</InputAdornment>,
                  }}
                />
              </InlineFormField>

              <InlineFormField label={t('Internal Transfer.Description')}>
                <Field fullWidth multiline rows={4} component={TextField} name="description" variant="filled" />
              </InlineFormField>
              ...
              <Grid item container spacing={2} justifyContent="flex-end">
                ...
                <Grid item>
                  <Button
                    disabled={isSubmitting || values.sourceAccount === null || values.destinationAccount === null || values.transferAmount === 0}
                    type="submit"
                    variant="contained"
                    color="primary"
                  >
