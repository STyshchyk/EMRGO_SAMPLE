...
const ExportButtons = forwardRef(({ tableKey, exportData = {}, hideExportButtons }, ref) => {
  ...
  const exportCSV = (e) => {
    ...
    if (listOfColumnNames && listOfRowValues) {
      ...
      if (tableKey === 'ssiTable') {
        csv = new CsvBuilder('counterparty_ssi.csv').addRow(listOfHeaderColumnNames).addRow(listOfColumnNames[0]).addRows(listOfRowValues).addRow(['']).addRow(['']).addRow(['Filters']);
      } else csv = new CsvBuilder(`${tableKey}.csv`).setColumns(listOfColumnNames[0]).addRows(listOfRowValues).addRow(['']).addRow(['']).addRow(['Filters']);
  ...
  return (
    !hideExportButtons && (
      <>
        <Grid item xs={6}>
          <Button variant="contained" disabled={transformedData?.length === 0} fullWidth color="primary" startIcon={<CloudDownloadIcon />} onClick={exportCSV}>
            CSV
          </Button>
        </Grid>

        <Grid item xs={6}>
          <Button variant="contained" disabled={transformedData?.length === 0} fullWidth color="primary" startIcon={<CloudDownloadIcon />} onClick={exportPDF}>
            PDF
          </Button>
        </Grid>
      </>
