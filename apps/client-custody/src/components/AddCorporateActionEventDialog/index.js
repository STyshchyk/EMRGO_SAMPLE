...
const AddCorporateActionEventDialog = ({ open, handleClose, selectedRow, setSelectedRow }) => {
  ...
  const externalSecuritiesList = useSelector(externalSecuritiesSelectors.selectExternalSecuritiesData);
  const currentEntityGroup = useSelector(authSelectors.selectCurrentEntityGroup);

  const { theme } = useTheme();
  const currentEntityGroupID = currentEntityGroup?.id;

  const corporateActionEventTypeOptions = dropdownDataOptions?.corporateActionEventType?.map((eventType) => ({ label: eventType.name, value: eventType.id }));
  const corporateActionEventStatusOptions = dropdownDataOptions?.corporateActionEventStatus?.map((eventStatus) => ({ label: eventStatus.name, value: eventStatus.id }));
  const externalSecurityOptionsList = generateExternalSecurityOptionsList(externalSecuritiesList);

  const selectedCorporateActionEvent = corporateActionEvents?.find(({ id }) => selectedRow?.id === id);
  ...
  if (isEdit) {
    corporateActionLinkedEventOptions = corporateActionEvents.filter((event) => event.id !== selectedCorporateActionEvent.id).map((linkEvent) => ({ label: linkEvent.eventId, value: linkEvent.id }));
  } else {
    corporateActionLinkedEventOptions = corporateActionEvents.map((linkEvent) => ({ label: linkEvent.eventId, value: linkEvent.id }));
  }
  ...
  useEffect(() => {
    if (selectedRow || selectedCorporateActionEvent) {
      ...
      if (!fetchingValues && data?.value && data?.value !== 'null' && data?.key === 'AddCorporateActionEventForm') {
  ...
  useEffect(() => {
    ...
    const fetchDropdownOptions = (payload) => dispatch(dropdownActionCreators.doFetchDropdownOptions(payload));
        ...
        >
          {({ values, handleSubmit, setFieldValue }) => (
            <form onSubmit={handleSubmit} onKeyDown={onKeyDown}>
              ...
              <DialogTitle id="form-dialog-title">{isEdit ? 'Edit Corporate Action Event' : 'Add Corporate Action Event'}</DialogTitle>
              <DialogContent>
                <Box mb={2}>
                  <Grid container>
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        ...
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="eventType" />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container className="my-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        ...
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="linkedEventId" />
                      </Grid>
                    </Grid>
                    <Grid container item justifyContent="flex-end">
                      <Grid item>
                        <Button variant="contained" color="secondary" onClick={() => setOpenRealtimeSecuritySearchDialog(true)}>
                    ...
                    <Grid container className="my-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        ...
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="securityId" />
                      </Grid>
                    </Grid>
                    <Grid container className="my-4">
                      <Grid item xs={12} md={6} lg={6} alignContent="flex-start">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        <FormControl className="w-full">
                          <Field
                            ...
                            variant="filled"
                            type="text"
                            value={values.externalSecuritySelectOption ? values.externalSecuritySelectOption.value?.longName : ''}
                            disabled
                          />
                        </FormControl>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        <Field fullWidth format="DD/MM/yyyy" inputVariant="filled" variant="dialog" placeholder="DD/MM/YYYY" component={KeyboardDatePicker} name="exDate" />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        <Field fullWidth format="DD/MM/yyyy" inputVariant="filled" variant="dialog" placeholder="DD/MM/YYYY" minDate={values.exDate} component={KeyboardDatePicker} name="recordDate" />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        ...
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="eventStatus" />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        ...
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="mandatoryOrVoluntary" />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        <Field fullWidth format="DD/MM/yyyy" inputVariant="filled" variant="dialog" placeholder="DD/MM/YYYY" component={KeyboardDatePicker} name="responseDeadline" />
                      </Grid>
                    </Grid>
                  </Grid>
                </Box>
              ...
              {openRealtimeSecuritySearchDialog && (
                <RealtimeSecSearchDialog
                  ...
                  handleSecurityResultItemSelect={(row) => {
                    const found = externalSecurityOptionsList.find((item) => item.value.id === row?.externalSecurityId);
