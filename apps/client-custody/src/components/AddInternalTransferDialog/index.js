...
const AddInternalTransferDialog = ({ open, handleClose }) => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    const createInternalTransfer = (payload) => dispatch(billingAndPaymentsActionCreators.doMoneyTransferInternal(payload));
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <Box mb={2}>
          <AddInternalTransferForm handleSubmit={handleSubmit} isSubmitting={isSubmitting} handleCloseDialog={() => handleClose()} />
        </Box>
      </DialogContent>
    </Dialog>
  );
