  ...
  const resultingData = data
    .map((arrItem) => {
      if (arrItem.length === 13) {
        const [settlementType, isin, currency, tradeDate, settlementDate, quantity, price, principalAmount, accruedInterest, settlementAmount, counterparty, counterpartySSI, internalTradeRef] =
          arrItem;
...
const ImportSecurityTradesTableDataDialog = ({ open, handleClose, setTableData, tableData, setHasImportedData }) => {
  ...
  const { getRootProps, getInputProps, isFocused, isDragAccept, isDragReject } = useDropzone({ onDrop, maxFiles: 1, accept: ALLOWED_FILE_TYPES });
