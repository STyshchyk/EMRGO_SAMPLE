...
import { reportDateRenderer, dateRenderer } from '../../constants/renderers';
...
const TableActionMenu = ({ handleCloseMenu, actions, anchorEl }) => (
  <Fragment>
    {Boolean(anchorEl) && (
      <Menu data-testid="security-trades-table-menu-list" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleCloseMenu} disableScrollLock>
        {actions
...
const generateSecurityTradesTableRowData = (i) => ({
  ...
  issuerCashAccountBalance: i.issuerCashAccount ? convertNumberToIntlFormat(i.issuerCashAccountBalance) : FALLBACK_VALUE,
...
const SecurityTradesTable = ({ anchorEl, handleCloseMenu, data, actions, setCurrentlySelectedRowData, showAllFilters, setAnchorEl, entityUserType, isLoading }) => {
  ...
  const isIntlSecTradeSettlementWorkflowEnabled = checkFeatureFlag(featureFlags.intlSecTradeSettlementWorkflow);
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      exportConfig: { render: (rowData) => dateFormatter(rowData?.entryDate, DEFAULT_DATE_TIME_FORMAT) },
      width: 150,
    },
    {
      id: 'tradeDate',
    ...
    },
    {
      ...
      hidden: ['INVESTOR'].includes(entityUserType),
      render: (rowData) => (rowData?.issuerSecuritiesAccountNo && convertNumberToIntlFormat(rowData?.issuerSecuritiesAccountNo)) || FALLBACK_VALUE,
    ...
    },
    {
      ...
      type: 'numeric',
      hidden: ['INVESTOR'].includes(entityUserType),
      render: (rowData) => (rowData?.issuerSecuritiesAccountBalance && convertNumberToIntlFormat(rowData?.issuerSecuritiesAccountBalance)) || FALLBACK_VALUE,
      width: 150,
    },
    {
      id: 'quantity',
  ...
  const listOfExternalSecurityNames = [...new Set(data.map(({ externalSecurity }) => externalSecurity?.name))].sort();
  const listOfISINValues = [...new Set(data.map(({ externalSecurity }) => externalSecurity?.isin))].sort();
    ...
    <Fragment>
      ...
      >
        <FilterProvider tableKey="custody_and_settlement">
          ...
          >
            <TableFiltersWrapper tableRef={tableRef} data={data} columns={tableColumns.slice(1, -1)} open={true} hideExportButtons>
              <Grid container spacing={2}>
                <Grid item xs={12} md={12} lg={6}>
                  <DateRangePicker name="entryDateRange" label={t('Filters.Entry Date')} defaultFilter="none" />
                </Grid>
                <Grid item xs={12} md={12} lg={6}>
                  <DateRangePicker name="tradeDateRange" label={t('Filters.Trade Date')} defaultFilter="none" />
                </Grid>

                <Grid item xs={12} md={12} lg={6}>
                  <DateRangePicker name="settlementDateRange" label={t('Filters.Settlement Date')} defaultFilter="none" />
                </Grid>
                {showAllFilters && (
                  <Fragment>
                    ...
                    <Grid item xs={12} md={6} lg={3}>
                      <DropdownFilter name="security" label="Security" options={externalSecurityOptionsList} />
                    </Grid>
                ...
                <Grid item xs={12} md={6} lg={3}>
                  <DropdownFilter name="status" label="Status" options={settlementStatusOptionsList} isMulti />
                </Grid>
          ...
          <FilterConsumer>
            {({ filters, filterColumns }) => {
              const filteredData = data
                .filter((row) => {
                  if (filters?.entryDateRange?.value?.startDate && filters?.entryDateRange?.value?.endDate) {
                ...
                .filter((row) => {
                  ...
                  if (filters?.tradeDateRange?.value?.startDate && filters?.tradeDateRange?.value?.endDate) {
                ...
                .filter((row) => {
                  ...
                  if (filters?.settlementDateRange?.value?.startDate && filters?.settlementDateRange?.value?.endDate) {
                ...
                .filter((row) => {
                  // Entity Filter
                  if (filters?.entity) {
                    return row.investor === filters?.entity?.value?.value || row.issuer === filters?.entity?.value?.value;
                  }
                  return true;
                })
                .filter((row) => {
                  ...
                  if (filters?.status) {
                    ...
                    const result = filters?.status?.value.map((v) => {
                      ...
                      return multiStatusKeys.includes(row.tradeSettlementOrSettlementInstructionStatus);
                    });
              ...
              return (
                <div data-testid="security-trades-table">
                  <MaterialTable
                    ...
                    isLoading={isLoading}
                  />
                  <TableActionMenu anchorEl={anchorEl} actions={actions} handleCloseMenu={handleCloseMenu} />
