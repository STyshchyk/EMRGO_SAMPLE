...
const EditableUI = ({ paymentAccounts, open, handleClose }) => {
  ...
  const [currentlySelectedPaymentAccountOption, setCurrentlySelectedPaymentAccountOption] = useState(null);
        ...
        <Grid container justifyContent="space-between">
          <Grid item>
            <Tabs value={value} onChange={handleChange} aria-label="simple tabs example" indicatorColor="primary">
              <Tab classes={{ root: classes.tab, disabled: classes.disabled }} disableRipple label="Your SSI" {...a11yProps(0)} />
              <Tab classes={{ root: classes.tab, disabled: classes.disabled }} disableRipple label="Wethaq SSI" {...a11yProps(1)} />
            </Tabs>
          </Grid>
          <Grid item>
            <IconButton
    ...
    <Dialog fullWidth maxWidth="md" open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
      <DialogTitle>
        <Grid container justifyContent="space-between">
          <Grid item>
            <Tabs value={value} onChange={handleChange} aria-label="simple tabs example" indicatorColor="primary">
              <Tab classes={{ root: classes.tab, disabled: classes.disabled }} disableRipple label="Investor SSI" {...a11yProps(0)} />
              <Tab classes={{ root: classes.tab, disabled: classes.disabled }} disableRipple label="Wethaq SSI" {...a11yProps(1)} />
            </Tabs>
          </Grid>
          <Grid item>
      ...
      >
        <TabPanel value={value} index={0}>
          <Grid container direction="column" spacing={2}>
            <Grid item>
              <Typography style={{ fontWeight: 'bold' }}>{`${selectedRowData?.investor}`}</Typography>
            </Grid>
...
const ViewSSIDialog = ({ paymentAccounts, open, handleClose, operationsMode, currentlySelectedRowData }) => (
  <Fragment>
    {operationsMode ? (
      <OperationsUI selectedRowData={currentlySelectedRowData} open={open} handleClose={handleClose} />
    ) : (
