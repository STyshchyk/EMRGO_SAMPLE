import { useEffect, useState, createRef, Fragment, useMemo } from 'react';
...
import { dateRenderer, idRenderer, currencyRenderer } from '../../constants/renderers';
...
const ClientRateCardViewDialog = ({ open, handleClose, selectedRow, modalType }) => {
  ...
  const sourceAccounts = generateAccountOptions(useSelector(cashManagementSelectors.selectAccountsData)?.data || []);

  const rateCard = rateCardConfig?.rate_card;
  const remotePaymentHolidays = useMemo(() => rateCardConfig?.payment_holiday?.filter((holidays) => holidays.is_active) || [], [rateCardConfig?.payment_holiday]);
  const remoteSafekeepingCharges = useMemo(() => rateCardConfig?.safe_keeping_charge?.filter((charges) => charges.is_active) || [], [rateCardConfig?.safe_keeping_charge]);
  const remoteTrxCharges = useMemo(() => rateCardConfig?.trx_charge?.filter((charges) => charges.is_active) || [], [rateCardConfig?.trx_charge]);
  ...
  const filteredAccounts = sourceAccounts?.filter((account) => account.value.entityGroupId === foundGroup?.id).filter((account) => account.value.currencyId === USDCurrency?.id);
  const selectedAccount = filteredAccounts.find((account) => account.id === rateCard?.billingAccount);

  const billingPeriodOptions = getDropdownValues(
    dropdownOptions?.reportingCycle?.filter((period) => period.key === 'monthly'),
    locale,
  );
  const selectedBillingPeriod = billingPeriodOptions.find((period) => period.value === rateCard?.billing_period_id);

  const transactionTypeOptions = {};
  getDropdownValues(
    dropdownOptions?.transactionType?.filter((type) => type.key === 'SETTLEMENT' || type.key === 'FX'),
    locale,
  ).forEach((type) => {
  ...
  useEffect(() => {
    ...
    const fetchAccounts = (payload) => dispatch(cashManagementActionCreators.doFetchAccounts(payload));
  ...
  const transactionTableColumns = [
    ...
    },
    {
      ...
      field: 'charge',
      initialEditValue: '',
      editComponent: (props) => <MuiTextField label="Charge ($)" variant="filled" value={props.value} onChange={(e) => props.onChange(e.target.value)} />,
  ...
  const safekeepingTableColumns = [
    ...
    },
    {
      ...
      field: 'band_start',
      editComponent: (props) => <MuiTextField label="Band Start ($)" variant="filled" value={props.value} onChange={(e) => props.onChange(e.target.value)} />,
    },
    {
      title: 'Txn Band End',
      field: 'band_end',
      editComponent: (props) => <MuiTextField label="Band End ($)" variant="filled" value={props.value} onChange={(e) => props.onChange(e.target.value)} />,
    },
    {
      title: 'Charge(BPS)',
      field: 'charge_in_bps',
  ...
  const processedSafeKeepingData = localSafekeepingCharges.map((band) => {
    ...
    return { ...band, band_end: band?.band_end === -1 ? Infinity : band?.band_end, type: concatedKey };
  });
  ...
  const handleDeleteSafekeeping = (requestObject, resolve) => {
    const filteredCharges = localSafekeepingCharges.filter((charge) => !requestObject.includes(charge.id));
  ...
  const holidayTableColumns = [
    ...
    },
    {
      ...
      initialEditValue: '',
      editComponent: (props) => <MuiTextField label="Narrative" variant="filled" value={props.value} onChange={(e) => props.onChange(e.target.value)} />,
    },
  ];
  ...
  const handleDeleteHoliday = (requestObject, resolve) => {
    const filteredHolidays = localPaymentHolidays.filter((holiday) => !requestObject.includes(holiday.id));
  ...
  const handleRateCardSubmit = (values) => {
    ...
    */
        ...
        if (index !== bandLength - 1) {
          ...
          if (band.band_end + 1 !== nextBand.band_start) {
            if (band.band_end >= nextBand.band_start) {
              // Overlap detected
              messages.push({ text: `Overlap between bands ${band.band_start}-${band.band_end} and ${nextBand.band_start}-${nextBand.band_end}`, icon: <SwapHorizIcon /> });
            } else {
              // Gap detected
              messages.push({ text: `Unaccounted gap between ${band.band_end} and ${nextBand.band_start}`, icon: <WrapTextIcon /> });
            }
          }
        }
      });
    ...
    if (groupedMessages.length > 0) {
      ...
      dispatch(billingActionCreators.doUpdateRatecardRequest({ requestPayload, clientRateCardId: selectedRow?.id }));
      handleClose();
    }
  };

  const handleApprove = () => {
    const foundApprove = dropdownOptions?.clientRateCardStatus.find((status) => status.key === 'active');
    ...
    dispatch(billingActionCreators.doApproveRatecardRequest({ requestPayload, clientRateCardId: selectedRow?.id }));
    handleClose();
  };

  return (
    <Fragment>
      <Dialog fullWidth open={open} onClose={handleClose} aria-labelledby="form-dialog-title" scroll="body" maxWidth="md">
        <Formik
          initialValues={{
        ...
        >
          {({ values, handleSubmit, setFieldValue }) => (
            <form onSubmit={handleSubmit}>
              <DialogTitle id="form-dialog-title"> {t('Client Rate Card.View Dialog.Client Rate Card', { type: changeCase.titleCase(modalType) })}</DialogTitle>
              ...
              <DialogContent>
                <Box mb={2}>
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={12} lg={6} container>
                      <Grid item xs={6} container alignContent="flex-start">
                        <Typography className="mt-4">{t('Client Rate Card.View Dialog.Client')}</Typography>
                      </Grid>
                      <Grid item xs={6} container alignContent="center" className="px-1">
                        <Field fullWidth disabled component={TextField} name="client" variant="filled" />
                      </Grid>
                    </Grid>
                    <Grid item xs={12} md={12} lg={6} container>
                      <Grid item xs={6} container alignContent="flex-start">
                        <Typography className="mt-4">{t('Client Rate Card.View Dialog.Billing Account')}</Typography>
                      </Grid>
                      <Grid item xs={6} container alignContent="center" className="px-1">
                    ...
                    <Grid item xs={12} md={12} lg={6} container>
                      <Grid item xs={6} container alignContent="flex-start">
                        <Typography className="mt-4">{t('Client Rate Card.View Dialog.Billing Period')}</Typography>
                      </Grid>
                      <Grid item xs={6} container alignContent="center" className="px-1">
                        <FormControl className="w-full">
                          <Select
                    ...
                    <Grid item xs={12} md={12} lg={6} container>
                      <Grid item xs={6} container alignContent="flex-start">
                        <Typography className="mt-4">{t('Client Rate Card.View Dialog.Minimum Charge (USD)')}</Typography>
                      </Grid>
                      <Grid item xs={6} container alignContent="center" className="px-1">
                        <Field disabled={modalType !== 'amend'} fullWidth component={TextField} name="minimumCharge" variant="filled" />
                      </Grid>
                    </Grid>
                  </Grid>
                  <Box my={4}>
                              ...
                              onRowAdd: (newData) =>
                                new Promise((resolve, reject) => {
                                  const schema = yup.object().shape({
                                    ...
                                    security_country_icsd_id: yup.string().required('Country/ICSD is required'),
                                  ...
                                  const requestObject = {
                                    ...
                                    security_country_icsd_id: newData?.security_country_icsd_id?.value,
                  ...
                  <Box my={4}>
                    <MaterialTable
                      title={
                        <Typography variant="h6" className={paymentHolidayOverlaps.length > 0 ? 'text-yellow-500' : ''}>
                          Payment Holidays
                    ...
                    {paymentHolidayOverlaps.length > 0 && showPaymentHolidayOverlapsError ? (
                      <Fragment>
                        ...
                        >
                          <AlertTitle>{t('Client Rate Card.View Dialog.OverlappingPeriodMessageKey', { count: paymentHolidayOverlaps.length })}</AlertTitle>
                          <ul>
