    ...
    return (
      <View
        style={[
          ...
          {
            width: colSpan ? `${width + defaultWidth || defaultWidth + defaultWidth}%` : `${width || defaultWidth}%`,
          },
        ]}
...
export const renderTableHeaders = (columns, setFontFamily) => {
  ...
  return columns.map((column) => {
    ...
    return (
      !hidden && (
        <View style={[styles.tableColHeader, { width: `${width || defaultWidth}%` }]} key={shortid.generate()}>
          <Text style={[styles.tableCellHeader, { fontFamily: setFontFamily }]}> {title}</Text>
        </View>
      )
    );
  ...
  const rows = data.map((rowData) => (
    <View style={styles.tableRow} key={shortid.generate()}>
      {columns.map((column) => {
        ...
        return (
          !hidden && (
            <View style={[styles.tableCol, { width: `${width || defaultWidth}%`, textAlign: align }]} key={shortid.generate()}>
      ...
      {slices.map((slice, index) => (
        <View wrap={false} style={styles.tableWrapper} key={shortid.generate()} break={index !== 0}>
          <View style={[styles.table]}>
            {filteredHeaderColumns.length > 0 && <View style={styles.tableRow}>{renderExtraHeader(filteredHeaderColumns, setFontFamily)}</View>}
