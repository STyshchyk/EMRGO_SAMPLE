...
const CashBalancesTable = ({ data, accounts }) => {
  ...
  const handleEntityChange = (selectedEntity) => {
    const filteredCashAccounts = cashAccountOpts
      .filter((account) => (selectedEntity ? account?.original?.group.id === selectedEntity.value : false))
      .map((account) => ({
        data: account,
        value: account.id,
        label: account.label,
    ...
    const filteredCurrencies = [...new Map(filteredCashAccounts?.map((cashAccount) => [cashAccount?.data?.original?.currency.id, cashAccount])).values()].map((currency) => {
    ...
    const tempSecurityAccountList = securityAccountOpts
      .filter((securityAccount) => (selectedEntity ? securityAccount.original.group.id === selectedEntity.data.id : true))
  ...
  const handleSecurityAccountChange = (selectedAccount) => {
    ...
    const tempEntitiesList = entityOpts
      .filter((entity) => (selectedAccount ? entity.id === selectedAccount.data.original.group.id : true))
      .map((entity) => ({ data: entity, value: entity.id, label: entity.label }));
        ...
        >
          <TableFiltersWrapper tableRef={tableRef} data={data} columns={columns} open={true} hideExportButtons>
            <Grid container spacing={2}>
              ...
              <Grid>
                <ReportingInfo cashAccount={currentlySelectedCashAccount} securityAccount={currentlySelectedSecurityAccount} />
              </Grid>
            </Grid>
          </TableFiltersWrapper>
        </div>
