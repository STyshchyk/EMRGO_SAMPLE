...
const TableFiltersWrapper = ({ columns, data, tableRef, exportData = {}, children, hideExportButtons, open = false }) => {
  ...
  return (
    <Fragment>
      <MuiAccordion TransitionProps={{ unmountOnExit: true }} elevation={0} expanded={expanded === 'filter-toolbar'} onChange={handleChange('filter-toolbar')}>
        <MuiAccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="filter-toolbar-content" id="filter-toolbar-header">
          <Grid container spacing={2} justifyContent="space-between">
            ...
            <Grid item xs={12} md={3} lg={3} spacing={2} container alignContent="center" justifyContent="space-between">
              <ExportButtons tableKey={tableKey} exportData={rest} ref={exportRef} hideExportButtons={hideExportButtons} />
            </Grid>

            <Grid item xs={12} sm={12} md={3} lg={3} container alignContent="center">
              <Button variant="outlined" onClick={openManageColumnsDialog} startIcon={<ViewColumnIcon />} color="secondary" size="large" fullWidth>
                {t('miscellaneous:Filters.Manage Columns')}
              </Button>
            </Grid>

            <Grid item xs={12} sm={12} md={12} lg={3}>
              <TableSearch name="search" tableRef={tableRef} label={t('miscellaneous:Filters.Search')} />
            </Grid>
          </Grid>
        </MuiAccordionSummary>
      ...
      {manageColumnStatus && <ManageColumnsDialog open={manageColumnStatus} closeDialog={closeManageColumnDialog} openResetColumnsDialog={openResetColumnsDialog} />}
      {resetColumnStatus && <ResetColumnsDialog open={resetColumnStatus} closeDialog={closeResetColumnDialog} />}
    </Fragment>
  );
};
