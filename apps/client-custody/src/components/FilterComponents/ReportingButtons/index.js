...
const ReportingButtons = ({ handleClear, handleFilter, disableApplyBtn, data }) => {
  ...
  return (
    <Grid item xs={12} md={6} lg={12} container alignContent="center" justifyContent="flex-end">
      ...
      <Box my={1} className="full-width">
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} md={3} lg={3}>
            <Button fullWidth disabled={disableApplyBtn} variant="contained" color="primary" onClick={() => handleFilter()} size="large">
              {t('reports:Cash Balances.Filters.Apply')}
            </Button>
          </Grid>

          <Grid item xs={12} md={3} lg={3}>
            <Button fullWidth disabled={data.length === 0} variant="outlined" color="primary" onClick={() => handleClear()} size="large">
      ...
      {manageColumnStatus && <ManageColumnsDialog open={manageColumnStatus} closeDialog={closeManageColumnDialog} />}
    </Grid>
  );
};
