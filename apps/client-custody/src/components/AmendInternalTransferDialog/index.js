...
import AddInternalTransferForm, { buildAddInternalTransferRequestPayload, generateEntityOptionsList, generateWethaqAccountOptionsList } from '../AddInternalTransferForm';
...
const AmendInternalTransferDialog = ({ open, handleClose, currentlySelectedRowData }) => {
  ...
  generatedInitialValues.sourceEntity = sourceEntitiesDropdown.find(({ value }) => currentlySelectedRowData?.sourceEntityId === value);
  generatedInitialValues.destinationEntity = destinationEntitiesDropdown.find(({ value }) => currentlySelectedRowData?.destinationEntityId === value);
  generatedInitialValues.sourceAccount = sourceAccountsDropdown.find(({ label }) => currentlySelectedRowData?.sourceAccountNumber === label);
  generatedInitialValues.destinationAccount = destinationAccountsDropdown.find(({ label }) => currentlySelectedRowData?.destinationAccountNumber === label);

  const handleSubmit = (values) => {
    const updateInternalJournal = (payload) => dispatch(journalsActionCreators.doUpdateInternalTransactions(payload));
    const fetchInternalTransactions = () => dispatch(journalsActionCreators.doFetchInternalTransactions());
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <Box mb={2}>
          <AddInternalTransferForm initialValues={generatedInitialValues} handleSubmit={handleSubmit} isSubmitting={isSubmitting} handleCloseDialog={() => handleClose()} />
        </Box>
      </DialogContent>
    </Dialog>
  );
