...
const SecuritiesHoldingsTable = ({ data, securitiesAccounts, disableDateFilter = false, entityUserType, setIsTradeDateHolding, isTradeDateHolding }) => {
  ...
  // selectors
  const isFetchingSecuritiesHoldings = useSelector(reportsSelectors.selectIsFetchingSecuritiesHoldings);
  const isFetchingTradeDatedSecuritiesHoldings = useSelector(reportsSelectors.selectIsFetchingTradeDatedSecuritiesHoldings);

  const isLoading = isTradeDateHolding ? isFetchingTradeDatedSecuritiesHoldings : isFetchingSecuritiesHoldings;
  ...
  const handleFetch = (filters) => {
    ...
    if (isAllEntitiesOptionSelected) {
      ...
      const addrs = securitiesAccounts.filter((account) => account?.group?.entity?.corporateEntityName === entity?.value?.label)[0];

      setEntityAddress(addrs);
    }

    const settlementDatedSecuritiesHoldings = (payload) => dispatch(reportsActionCreators.doFetchSecuritiesHoldings(payload));
    const tradeDatedSecuritiesHoldings = (payload) => dispatch(reportsActionCreators.doFetchTradeDatedSecuritiesHoldings(payload));

    const fetchSecuritiesHoldings = isTradeDateHolding ? tradeDatedSecuritiesHoldings : settlementDatedSecuritiesHoldings;
        ...
        >
          <TableFiltersWrapper tableRef={tableRef} data={data} columns={columns} open={true} hideExportButtons>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6} lg={3}>
                {!disableDateFilter && <DatePicker name="date" label="Date" defaultFilter={moment()} />}
              </Grid>
              ...
              <Grid item xs={12} md={6} lg={3}>
                <DropdownFilter name="positionType" label="Position Type" options={positionTypeOptionsList} />
              </Grid>
              <Grid item xs={12} md={6} lg={3}>
              ...
              <Grid item xs={12} container>
                ...
                <Typography variant="subtitle1">{` ${
                  entityAddress?.group?.addresses && !allEntitiesOptionSelected ? formatAddress(entityAddress?.group?.addresses) : t('reports:Cash Balances.NA')
                }`}</Typography>
              </Grid>
            </Grid>
          </TableFiltersWrapper>
