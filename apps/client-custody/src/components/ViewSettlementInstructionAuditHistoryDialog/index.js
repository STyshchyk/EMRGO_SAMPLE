...
import { DEFAULT_DATE_TIME_FORMAT, DEFAULT_DATE_FORMAT } from '../../constants/datetime';
...
const generateSIAuditHistoryStatement = (item) => {
  ...
  if (auditType === 'Action') {
    ...
    if (auditColumnLabel) {
      return `${constantCase(auditColumnLabel)} changed from ${formatSIFieldValue(prevStatus, auditColumnLabel)} to ${formatSIFieldValue(newStatus, auditColumnLabel)} by ${userFullName}`;
    }
  }
...
const ViewSettlementInstructionAuditHistoryDialog = ({ open, handleClose, currentlySelectedRowData }) => {
  const dispatch = useDispatch();
  const ref = useRef();
  const settlementId = currentlySelectedRowData?.id;

  // selectors
  const isFetchingSettlementInstructionAuditData = useSelector(paymentAndSettlementSelectors.selectIsFetchingSettlementInstructionAuditData);
  const settlementInstructionAuditHistoryDataList = useSelector(paymentAndSettlementSelectors.selectSettlementInstructionAuditHistoryDataList);

  const tableData = settlementInstructionAuditHistoryDataList.map((item) => generateSIAuditHistoryTableRowData(item));

  useEffect(() => {
    const fetchSettlementInstructionAuditData = (payload) => dispatch(paymentAndSettlementActionCreators.doFetchSettlementInstructionAuditData(payload));
    const resetSettlementInstructionAuditData = () => dispatch(paymentAndSettlementActionCreators.doResetSettlementInstructionAuditData());
  ...
  const handleCSVExportClick = (data) => {
    ...
    const csvBuilder = new CsvBuilder(csvFileName).setColumns(listOfColumnNames).addRows(listOfRows);
  ...
  const columns = [
    {
      ...
      exportConfig: { render: (rowData) => dateFormatter(rowData?.auditTimestamp, DEFAULT_DATE_TIME_FORMAT) },
    },
    {
      id: 'description',
      title: 'Description',
  ...
  return (
    <Fragment>
      ...
      >
        <StyledDialogHeader title="View Settlement Instruction Audit History" handleClose={handleClose} />
        <DialogContent>
          <Box mb={2}>
            <Grid
              container
            ...
            <Grid item md={12}>
              <SettlementInstructionAuditHistoryTable isLoading={isFetchingSettlementInstructionAuditData} data={tableData} columns={columns} />
            </Grid>
            <Grid
              item
              container
      ...
      <ReportingTablePDFExporter ref={ref} title="Audit History">
        <View style={pdfStyles.content}>
          ...
          <Filters filters={exportFilters} styleProps={{ filterPaddingBottom: 5, contentPaddingBottom: 0 }} />
        </View>
