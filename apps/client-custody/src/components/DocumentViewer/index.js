...
const DocumentViewer = ({ documentURL }) => {
  ...
  const memoizedDocumentObject = useMemo(() => ({ uri: documentURL, fileType: documentURL?.split('?', 1)[0].split('.').pop() ?? undefined }), [documentURL]);

  if (!SUPPORTED_DOCUMENT_FILE_TYPES.includes(memoizedDocumentObject.fileType))
    return <strong style={{ color: 'red' }}>{t('components:Document Viewer.Error: Document File Type Not Supported')}</strong>;
