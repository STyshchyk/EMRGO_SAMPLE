...
const UserInfoModal = ({ open, onClose, selectedUserInfo }) => {
  ...
  let userInfoRows = [
    ...
    createData(
      ...
      user?.relationshipManagers ? user?.relationshipManagers.map((manager, index) => `${index !== 0 ? ',' : ''}${manager.firstName || ''} ${manager.middleName || ''} ${manager.lastName || ''}`) : '',
    ),
  ];
  ...
  userInfoRows = [
    ...
    ...regionSwitcher({
      sa: [
        ...
        createData(t('onboarding:Info.Passport Expiry Date'), dateFormatter(user?.individualKyc?.passportExpiry, dateFormat)),
      ],
  ...
  const userTimelineRows = [
    ...
    createData(t('onboarding:Info.Date of Invitation ( reminder after 3 days )'), dateFormatter(userTimeline?.dateOfInvitation, dateFormat)),
    createData(t('onboarding:Info.Date of T&C Accepted'), dateFormatter(userTimeline?.dateOfTnCAcceptance, dateFormat)),
    createData(t('onboarding:Info.Date of Onboarding'), dateFormatter(userTimeline?.dateOfOnboarding, dateFormat)),
    createData(t('onboarding:Info.Date of Entity KYC requested'), dateFormatter(userTimeline?.dateOfKycRequest, dateFormat)),
    createData(t('onboarding:Info.Date of Entity KYC start'), dateFormatter(userTimeline?.dateOfKycStart, dateFormat)),
    createData(t('onboarding:Info.Date of Entity KYC complete/User Approval'), dateFormatter(userTimeline?.dateOfKycComplete, dateFormat)),
