...
const ChangeSettlementInstructionStatusDialog = ({ requestedSettlementInstructionStatus, open, handleClose, currentlySelectedRowData }) => {
  ...
  const changeSettlementInstructionStatus = (payload) => dispatch(paymentAndSettlementActionCreators.doChangeSettlementInstructionChange(payload));
  ...
  switch (requestedSettlementInstructionStatus) {
    case settlementInstructionStatusEnum.UNMATCHED:
      settlementStatusReasonSelectOptionsList = generateDropdownOptionsList(dropdownOptions?.settlementInstructionUnmatchedReason);
      break;

    case settlementInstructionStatusEnum.FAILED:
      settlementStatusReasonSelectOptionsList = generateDropdownOptionsList(dropdownOptions?.settlementInstructionFailedReason);
      break;

    case settlementInstructionStatusEnum.REJECTED:
      settlementStatusReasonSelectOptionsList = generateDropdownOptionsList(dropdownOptions?.settlementInstructionRejectedReason);
      break;
  ...
  return (
    ...
    >
      <StyledDialogHeader title={`Change SI status to ${requestedSettlementInstructionStatus}?`} handleClose={handleClose} />
      <DialogContent>
        <Box mb={2}>
          <Grid
            container
