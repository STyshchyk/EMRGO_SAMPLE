...
const MaterialTableOverflowMenu = ({ id, actions, anchorEl, setAnchorEl, selectedRow }) => (
  <Menu id={id || 'simple-menu'} data-testid="overflow-menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)} disableScrollLock>
    {actions.map((action) => {
      const isHidden = typeof action.hidden === 'function' ? action.hidden(selectedRow) : action.hidden;
      return isHidden ? (
        ''
      ) : (
        <MenuItem
          ...
          disabled={typeof action.disabled === 'function' ? action.disabled(selectedRow) : action.disabled}
