import { Fragment, useState, createRef, useRef, useMemo } from 'react';
...
import { issuanceColumns, tradeColumns, accountColumns, preSetColumns, defaultColumnsSelected, filteredColumns } from '../../constants/paymentAndStatuses/blotterColumns';
...
const Blotter = ({ keyedData, parameters, blotterId, updateSingleBlotter, deleteSingleBlotter, blotterDropdowns, settleTrade, currentEntityGroup, approveTrade }) => {
    ...
    const exportedData = data.map((rowData) =>
      columns.map((columnDef) => {
        ...
        if (columnDef.type === 'date') {
          returnable = columnDef.render ? columnDef.render(rowData) : blotterRef.current.dataManager.getFieldValue(rowData, columnDef);
  ...
  const reInitColumns = (storedColumns) => {
    const reInitialisedColumns = storedColumns.map((column) => {
      const defaultColumn = [...issuanceColumns, ...tradeColumns, ...accountColumns].find((value) => {
  ...
  const getItemStyle = (isDragging) => (isDragging ? style['columnManager__column--active'] : style['columnManager__column--inactive']);

  const getListStyle = (isDraggingOver) => (isDraggingOver ? style['columnManager__list--active'] : style['columnManager__list--inactive']);
    ...
    <Fragment>
        ...
        <MuiPickersUtilsProvider utils={MomentUtils} locale={theme.locale.altLocale}>
          <Formik
            initialValues={{
              ...
              shownColumns: filters.shownColumns ? reInitColumns(filters.shownColumns) : defaultColumns.shown,
            ...
            {({ submitForm, values, setFieldValue }) => {
              ...
              const columnDefinitions = values.shownColumns
                ...
                .map((column) => ({
                  ...
                  title: column.columnName ? t(`blotter:Columns.${column.columnName}`) : t(`blotter:Columns.${column.title}`),
              ...
              const handleRemoveCurrencySelection = (e, key) => {
                e.stopPropagation();
                const newCurrencyList = values.currency.filter((currency) => currency.value !== key);
              ...
              const handleRemoveCountrySelection = (e, key) => {
                e.stopPropagation();
                const newCountryList = values.issuerCountry.filter((country) => country.value !== key);
              ...
              const moveHiddenColumns = () => {
                setFieldValue('shownColumns', [...values.shownColumns, ...values.hiddenColumns], false);
                setFieldValue('hiddenColumns', [], false);
              };

              const moveShownColumns = () => {
                setFieldValue('hiddenColumns', [...values.hiddenColumns, ...values.shownColumns], false);
              ...
              const handlePresetColumnSelected = (event, option) => {
                ...
                const partitionedArray = partition(allColumns, (column) => option.columns.includes(column.field));
              ...
              const onDragEnd = (result) => {
                ...
                if (source.droppableId === destination.droppableId) {
                  ...
                  const currentResult = move(getList(source.droppableId), getList(destination.droppableId), source, destination);
              ...
              const filteredData = keyedData
                ...
                .filter((row) => {
                  ...
                  return moment(row.sukuk.issueDate).isBetween(settlementFromDate, settlementToDate, undefined, []);
                })
                .filter((row) => {
                ...
                .filter((row) => {
                  if (values.entityName !== '') {
                    return row.investorEntity.corporateEntityName.toLowerCase().search(values.entityName.toLowerCase()) !== -1;
                  }
                  return true;
                })
                .filter((row) => {
                  if (values.entityId !== '') {
                    return row.investorEntity.id.toLowerCase().search(values.entityId.toLowerCase()) !== -1;
                  }
                  return true;
                })
                .filter((row) => {
                ...
                .map((row) => ({
                  ...
                  sukuk: {
                    ...row.sukuk,
                    profitRateTermsName: { name: t(`statuses:Profit Rate Terms.${row?.sukuk?.profitRateTermsName?.name}`) },
                    frequencyName: { name: t(`statuses:Frequency.${row?.sukuk?.frequencyName?.name}`) },
                  ...
                  readyToSettle: { value: row.readyToSettle, label: t(`statuses:Ready to Settle.${row.readyToSettle ? 'Yes' : 'No'}`) },
                }));
              ...
              return (
                <Form>
                  <Box p={2} className={style.blotter__wrapper}>
                    <Grid container justifyContent="space-between" className={style.blotter__card__header}>
                      ...
                      <Grid item xs={4} container justifyContent="flex-end">
                        <div>
                          <ButtonGroup variant="outlined" color="secondary" ref={anchorRef} aria-label="split button">
                            <Button variant="outlined" startIcon={<ViewColumnIcon />} color="secondary" onClick={() => selectDefaultColumns()}>
                          ...
                          <Popper open={openManageColumns} anchorEl={anchorRef.current} role={undefined} transition disablePortal className={style.groupedButton__popper}>
                                  ...
                                  <ClickAwayListener onClickAway={handlePresetColumnSelection}>
                                    <MenuList id="split-button-menu">
                                      {preSetColumns.map((option) => (
                                        <MenuItem key={option.key} onClick={(event) => handlePresetColumnSelected(event, option)}>
                        ...
                        <Box p={1}>
                          <Grid container spacing={2}>
                            <Grid item xs={12} md={6} lg={3} container>
                              <Grid container justifyContent="space-between">
                                <Grid item xs={9} container alignContent="center" justifyContent="flex-start">
                                  <Typography variant="body1" className="bold clickable" aria-controls="simple-menu" aria-haspopup="true" onClick={handleDateRangePickerOpen}>
                                    {t('blotter:Filters.Trade Date')}
                                  </Typography>
                                  <KeyboardArrowDownIcon className="bold clickable" aria-controls="simple-menu" aria-haspopup="true" onClick={handleDateRangePickerOpen} />
                                  <Menu id="simple-menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleDateRangePickerClose}>
                                    <MenuItem
                                      onClick={() => {
                                        setDateRange('today');
                                      }}
                            ...
                            <Grid item xs={12} md={6} lg={3} container>
                              <Grid container justifyContent="space-between">
                                <Grid item xs={9} container alignContent="center" justifyContent="flex-start">
                                  <Typography variant="body1" className="bold clickable" aria-controls="simple-menu" aria-haspopup="true" onClick={handleSettlementDateRangePickerOpen}>
                                    {t('blotter:Filters.Settlement Date')}
                                  </Typography>
                                  <KeyboardArrowDownIcon className="bold clickable" aria-controls="simple-menu" aria-haspopup="true" onClick={handleSettlementDateRangePickerOpen} />
                                  <Menu id="settlement-menu" anchorEl={settlementAnchorEl} keepMounted open={Boolean(settlementAnchorEl)} onClose={handleSettlementDateRangePickerClose}>
                                    <MenuItem
                                      onClick={() => {
                          ...
                          <Grid container justifyContent="flex-end">
                            <Button variant="contained" color="primary" startIcon={<CloudDownloadIcon />} onClick={() => exportToFile()}>
                    ...
                    />
                    <MaterialTableOverflowMenu actions={actions} anchorEl={menuAnchorEl} setAnchorEl={setMenuAnchorEl} selectedRow={selectedTrade} />
                  </Box>
                  <Dialog fullWidth maxWidth="md" open={columnModalOpen} onClose={() => setColumnModalOpen(false)} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                    <DialogTitle id="alert-dialog-title">{t('blotter:Manage Columns Dialogue.Manage Columns')}</DialogTitle>
                    <DialogContent>
                        ...
                        <Grid container justifyContent="space-between">
                          <Grid item xs={5}>
                            <Grid container justifyContent="space-between" alignContent="center">
                              ...
                              <Tooltip title={t('blotter:Manage Columns Dialogue.Move all Hidden Columns')} placement="top" aria-label="move hidden">
                                <IconButton
                            ...
                            <Droppable droppableId="hidden">
                              {(provided, snapshot) => (
                                <div ref={provided.innerRef} className={cx(style.columnManager__list, getListStyle(snapshot.isDraggingOver))}>
                                  {values.hiddenColumns
                                    .filter((column) => !entityTypeFilteredColumns.includes(column.field))
                                    .map((item, index) => (
                                      <Draggable key={`${item.field}-${item.title}}`} draggableId={`${item.field}-${item.title}}`} index={index}>
                                        {(currentProvided, currentSnapshot) => (
                                          <div
                                            ...
                                            className={cx(style.columnManager__column, style['columnManager__column--hidden'], getItemStyle(currentSnapshot.isDragging))}
                                          >
                          ...
                          <Grid item xs={2} container justifyContent="center" alignContent="flex-start">
                            <div className={style.columnManager__swap__wrapper}>
                              <IconButton
                                ...
                                size="large">
                                <Tooltip title={t('blotter:Manage Columns Dialogue.Swap all Columns')} placement="top" aria-label="swap columns">
                                  <SwapHorizontalCircleIcon color="primary" fontSize="large" className={style.columnManager__swap__icon} />
                                </Tooltip>
                              </IconButton>
                            </div>
                          </Grid>
                          <Grid item xs={5}>
                            <Grid container justifyContent="space-between" alignContent="center">
                              <Tooltip title={t('blotter:Manage Columns Dialogue.Move all Shown Columns')} placement="top" aria-label="move shown">
                                <IconButton
                                  aria-label="move columns"
                                  onClick={() => {
                                    moveShownColumns();
                                      ...
                                      <Draggable key={`${item.field}-${item.title}}`} draggableId={`${item.field}-${item.title}}`} index={index}>
                                        {(currentProvided, currentSnapshot) => (
                                          <div
                                            ...
                                            {...currentProvided.dragHandleProps}
                                            style={{ ...currentProvided.draggableProps.style }}
                                            className={cx(style.columnManager__column, style['columnManager__column--shown'], getItemStyle(currentSnapshot.isDragging))}
                                          >
                                            {t(`blotter:Columns.${item.title}`)}
                                          </div>
                                        )}
      ...
      {selectedInvestor.investorEntity ? (
        <Dialog open={openDialogue} onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
          <DialogTitle id="alert-dialog-title">{t('blotter:Settle Payment Dialogue.Settle payments for', { name: selectedInvestor.investorEntity.corporateEntityName })}</DialogTitle>
          <DialogContent>
      ...
      {selectedInvestor.investorEntity ? (
        <Dialog open={openApproveDialogue} onClose={handleApproveClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
          <DialogTitle id="alert-dialog-title">{t('blotter:Approve Payment Dialogue.Approve trade for', { name: selectedInvestor.investorEntity.corporateEntityName })}</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
