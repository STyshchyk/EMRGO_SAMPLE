...
const TableActionMenu = ({ handleCloseMenu, actions, anchorEl }) => {
  ...
  return (
    <Fragment>
      {Boolean(anchorEl) && (
        <Menu id="ss-table-action-menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleCloseMenu} disableScrollLock>
          {actions.map((action) => (
            <MenuItem key={action.id} disabled={action.disabled} onClick={action.onClick}>
              <Typography variant="inherit">{t(`External Securities.Context Menu.${action.label}`)}</Typography>
            </MenuItem>
          ))}
        </Menu>
      )}
...
const generateExternalSecuritiesListTableRowData = (i) => ({
  ...
  attributes: i.attributes
    ...
    .map((item) => ({ identifierId: item.identifierId, value: item.value, securityIdTypeName: item.match?.value })),
});

const ExternalSecuritiesTable = ({ anchorEl, handleCloseMenu, data, actions, setCurrentlySelectedRowData, setAnchorEl, setOpenAddSecurityDialog, setOpenAddEquitySecurityDialog }) => {
  ...
  const extSecuritiesColumns = [
    ...
    },
    {
      ...
      customFilterAndSearch: (term, rowData) => term === dateFormatter(rowData?.maturityDate, DEFAULT_DATE_FORMAT),
