...
const AddCounterpartySSIDialog = ({ open, handleClose, selectedRow, setSelectedRow }) => {
  ...
  useEffect(() => {
    dispatch(
      counterpartyActionCreators.doFetchDropdowns({
        options: ["settlementLocation", "deliveryAgentIdTypes", "sellerIdTypes"],
      })
    );
  ...
  const buildRequestPayload = (values) => {
    ...
    const sellerIdentifier =
      requestPayload.sellerOrBuyerIdentifier === "" ? null : requestPayload.sellerOrBuyerIdentifier;
    const safekeepingAccount =
      requestPayload.safekeepingAccount === "" ? null : requestPayload.safekeepingAccount;
    ...
    return {
      ...
      deliveryAgentIdentifierTypeId,
      deliveryAgentIdentifier,
      sellerIdentifierTypeId,
      sellerIdentifier,
      safekeepingAccount,
    };
  };
  ...
  const getDeliveryOrReceiveAgentIdTypeOptionsList = (settlementLocationId) => {
    if (settlementLocationId) {
      const filteredOptions = dropdowns?.deliveryAgentIdTypes?.filter(
        (item) => item?.parentId === settlementLocationId?.toString()
      );
  ...
  const getSellerOrBuyerIdTypeOptionsList = (settlementLocationId) => {
    if (settlementLocationId) {
      const filteredOptions = dropdowns?.sellerIdTypes?.filter(
        (item) => item?.parentId === settlementLocationId?.toString()
      );
      const options = getDropdownValues(filteredOptions, locale);
      return options;
  ...
  useEffect(() => {
    if (selectedRow || selectedCounterpartySSI) {
      setInitialValues({
        ...
        settlementLocation: selectedRow?.settlementLocation
          ? selectedCounterpartySSI.settlementLocationId
          : null,
        deliveryOrReceiveAgentIdType: selectedRow?.deliveryOrReceiveAgentIdType
          ? selectedCounterpartySSI.deliveryAgentIdentifierTypeId
          : null,
        deliveryOrReceiveIdentifier: selectedRow?.deliveryOrReceiveIdentifier || "",
        sellerOrBuyerIdType: selectedRow?.sellerOrBuyerIdType
          ? selectedCounterpartySSI?.sellerIdentifierTypeId
          : null,
    ...
    >
      ...
      >
        {({ handleSubmit, setFieldValue, values }) => {
          ...
          const deliveryOrReceiveAgentIdTypeOptionsList =
            getDeliveryOrReceiveAgentIdTypeOptionsList(values?.settlementLocation?.value);
          const sellerOrBuyerIdTypeOptionsList = getSellerOrBuyerIdTypeOptionsList(
            values?.settlementLocation?.value
          );
                    ...
                    <Grid item xs={12} lg={12} container className="mt-4">
                      ...
                      >
                        ...
                        <ErrorMessage
                          ...
                          color="error"
                          className="ml-4"
                          name="deliveryOrReceiveAgentIdType"
                        />
                      </Grid>
                    </Grid>
