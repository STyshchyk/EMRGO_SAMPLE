...
const ReleaseFundsModal = ({ isModalOpen, setIsModalOpen, selectedTransaction, handleRelease }) => {
  ...
  const payload = {
    ...
    destinationAccountId: selectedTransaction.assignedAccount ? selectedTransaction.assignedAccount.id : null,
  ...
  return (
    <Dialog open={isModalOpen} onClose={handleClose} aria-labelledby="send-onboarding-invitation" maxWidth="sm" fullWidth>
      <DialogTitle id="form-dialog-title">{t('cash_management:Incoming Payments.Modals.Release Payment')}</DialogTitle>
      <DialogContent>
        <Box className={style.detailsWrapper}>
          ...
          <br />
          <Typography variant="body1">
            Destination Account : {selectedTransaction.assignedAccount ? `${selectedTransaction.assignedAccount.accountNo} ( ${titleCase(selectedTransaction?.assignedAccount?.type)} ) ` : 'N/A'}
          </Typography>
