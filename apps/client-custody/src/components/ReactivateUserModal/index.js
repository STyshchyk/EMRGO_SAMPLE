    ...
    <Fragment>
      <Formik
        ...
        onSubmit={(values, { setSubmitting }) => {
          ...
          reactivateEntity(requestPayload, selectedRow?.user ? selectedRow.user.id : selectedRow.id, selectedRow?.entityId);
        }}
      >
        {({ handleSubmit }) => (
          <Form>
            <Dialog fullWidth open={open} onClose={onClose} aria-labelledby="form-dialog-title" PaperProps={{ className: 'overflow-y-visible' }}>
              <DialogTitle id="edit-group-types-form-dialog-title">{t('onboarding:Forms.Are you sure you want to reactivate user?')}</DialogTitle>
