        ...
        return (
          <Form>
            <Grid container spacing={2} alignItems="center">
              <Grid item xs={6}>
                <Typography>{t('Upload document evidencing payment ')}(SWIFT, SARIE, UAEFTS, etc.)</Typography>
              </Grid>
              <Grid item xs={6}>
                <FileUploadField fullWidth name="paymentConfirmationFileName" acceptableFileTypes={ACCEPTABLE_FILE_TYPES} />
              </Grid>
              <Grid item container justifyContent="flex-end">
                <Grid item>
                  <Button type="submit" variant="contained" color="primary" data-testid="submit" disabled={isUploading || !isValid}>
...
const UploadPaymentConfirmationDialog = ({ open, handleClose, currentlySelectedRowData }) => {
  ...
  const handleSubmit = (values, action) => {
    const uploadPaymentConfirmationFile = (payload) => dispatch(securitiesServicesActionCreators.doUploadFile(payload));
  ...
  return (
    <Dialog maxWidth="lg" open={open} onClose={handleClose} aria-labelledby="upload-payment-confirmation-file-form-dialog">
      <DialogTitle>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            ...
            >
              {` ${t('Upload Payment Confirmation')} - ${currentlySelectedRowData?.externalSecurity?.issuanceName}`}
            </Typography>
          </Grid>
          <Grid item>
            <IconButton
