...
import EntityGroupSelector from '../EntityGroupSelectorExp';
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// !!! THE PAGE COMPONENT HAS BEEN MODIFIED TO INCLUDE ENTITY GROUP SELECTOR - 13/3/2023
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
const TwoFactorAuthentication = () => {
  ...
  const { isMFACodeVerified, MFAActive, firstName } = authenticatedUserObject;
  ...
  return (
    <Grid container justifyContent="center" alignContent={'center'} className={style.container}>
      {MFAActive ? (
        <Grid item xs={12} md={6} lg={4} container direction="column" justifyContent="flex-start" alignContent="center" className="py-4 px-16">
          <Formik
            initialValues={{ otp: '' }}
            onSubmit={(values) => {
              submitToken(values);
            ...
            {({ values, setFieldValue }) => (
              <Fragment>
                <Form className={style.form} data-testid="login-form">
                  <Box mt={2} mb={4}>
                    <Typography align="center" variant="h5" className="my-4">
                      {t('auth:Welcome back', { name: firstName })}
                    </Typography>
                  </Box>
                  <EntityGroupSelector />
                  <Box mb={4}>
                    <Typography align="center" className="my-4">
                      {t('auth:Enter the security code generated by your two-factor authentication app')}. {t('auth:You will be provided a new security code every 30 seconds')}
                    </Typography>
                    <OtpInput
                      value={values.otp}
                      ...
                      isInputNum
                      numInputs={6}
                      shouldAutoFocus={numberOfOwnEntityGroups === 1}
                      // separator={<Box m={2} />}
                  ...
                  <Box mb={2}>
                    <Button type="submit" disabled={values.otp.length !== 6} fullWidth variant="contained" color="primary" id="otpButton" data-testid="otpButton">
        ...
        <Grid container justifyContent="center" alignContent="flex-start" className={style.container}>
          <Grid item xs={12} md={5} container direction="column" justifyContent="flex-start" alignContent="center" className="py-4 px-16">
            {isRequesting ? (
              <CircularProgress />
            ) : (
              <Fragment>
                <Box mt={2} mb={2}>
                  ...
                  <Typography align="center">{t('auth:Scan the following QR Code with a two-factor authentication app on your phone')}</Typography>
                </Box>
          ...
          <Grid item xs={12} md={5} container direction="column" justifyContent="flex-start" alignContent="center" className={style.container__section}>
            <Formik
              initialValues={{ otp: '' }}
              onSubmit={(values) => {
                submitToken(values);
                ...
                <Fragment>
                  <Form className={style.form}>
                    ...
                    <Box>
                      ...
                      />
                      <Box mb={2}>
                        <Button type="submit" disabled={values.otp.length !== 6} fullWidth variant="contained" color="primary" data-testid="otpButton">
                          {t('auth:Buttons.Setup')}
                        </Button>
                      </Box>
                      <Grid container justifyContent="space-between">
                        <Button className="text-gray-500" onClick={() => handleBackToLoginClick(true)}>
                          {t('auth:Buttons.Back to Login')}
                        </Button>
                        <Button onClick={() => setRaiseTicketModal(true)} color="primary">
                          {t('auth:Buttons.Raise Support Ticket')}
      ...
      >
        {({ handleSubmit }) => (
          <form onSubmit={handleSubmit} noValidate>
            <Dialog open={showRaiseTicketModal} onClose={() => setRaiseTicketModal(false)} maxWidth="sm" fullWidth>
              <DialogTitle id="alert-dialog-title">{t('auth:Reset 2FA Modal.Raise 2FA Ticket')}</DialogTitle>
              <DialogContent dir={locale.rtl ? 'rtl' : 'ltr'}>
                <DialogContentText id="alert-dialog-description">
                  {t('auth:Reset 2FA Modal.Do you want to raise a ticket to Emrgo support to reset your 2 Factor Authentication credentials')}.
                  {t('auth:Reset 2FA Modal.Only do this when you have lost access to your authenticator app')}.{t('auth:Reset 2FA Modal.Admin will use the file you upload to verify your account')}.
                </DialogContentText>
                <Box>
                  <FileUploadField
              ...
              <DialogActions>
                ...
                <Button disabled={isUploadingIDDocument} onClick={raiseTicket} variant="contained" color="primary">
