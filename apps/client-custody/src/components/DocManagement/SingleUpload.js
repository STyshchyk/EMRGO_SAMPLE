...
const SingleUpload = ({ entry, uploadStatus, handleRemoveFile, handleUploadFile, index, allCategories, availableCategories }) => {
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <div style={{ marginLeft: 30, display: 'flex', alignItems: 'center' }}>
              ...
              {!editDisabled ? <input value={fileName} type="text" className={style.inlineText} onChange={(e) => handleFileNameChange(e.target.value)} /> : `${fileName}`}
            </div>
            <div style={{ marginLeft: 30, flex: 1, display: 'flex', alignItems: 'center' }}>
              {t('components:Single Upload.Category')}:{' '}
              <div style={{ width: 200 }}>
                {!editDisabled ? (
                  <ClearSelect name="category" isClearable={false} isSearchable={false} value={fileCategory} onChange={handleFileCategoryChange} options={availableCategories} />
                ) : (
        ...
        {uploadState === 1 ? (
          ...
          <Fragment>
            ...
            <IconButton
              ...
              size="large">
              {uploadState === 1 ? <RemoveCircleOutline fontSize="inherit" /> : <Delete fontSize="inherit" />}
            </IconButton>
          </Fragment>
        )}
      </div>
