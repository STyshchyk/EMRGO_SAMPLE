...
const administration = {
  ...
  requiredEntityTypes: ['ADMINISTRATOR', 'ARRANGER', 'FIDUCIARY', 'INVESTOR', 'ISSUER', 'LEGAL_COUNSEL', 'OBLIGOR', 'VISITOR', 'EMRGO_SERVICES'],
};
...
const issuance = {
  ...
  requiredEntityTypes: ['ARRANGER', 'CO_ARRANGER', 'FIDUCIARY', 'INVESTOR', 'ISSUER', 'LEGAL_COUNSEL', 'OBLIGOR'],
};
...
const blotter = {
  ...
  requiredEntityTypes: ['ARRANGER', 'CO_ARRANGER', 'INVESTOR', 'ISSUER', 'OBLIGOR', 'EMRGO_SERVICES'],
};
  ...
  const filteredRoutingConfigs = Object.keys(routingConfigs).filter((key) =>
    currentListOfAcls?.some((acl) => {
      if (routingConfigs[key]) {
        ...
        if (!currentRoutingConfig.acls.length && !currentRoutingConfig.requiredEntityTypes.length) return true;

        if (currentRoutingConfig.requiredEntityTypes) {
          return currentRoutingConfig.acls.includes(acl) && currentRoutingConfig.requiredEntityTypes.includes(entityType);
        }
      ...
      {filteredRoutingConfigs.map((item) => {
        ...
        return (
          <ListItem key={displayName}>
            <ButtonBase className={cx(locale.rtl ? style.drawer__link__wrapper__right : style.drawer__link__wrapper__left)} component={RouterLink} to={homeUrl}>
              <Paper className={cx(style.drawer__link, matchURL ? style['drawer__link--active'] : '')}>
...
const DashboardSidebar = ({ open }) => {
  ...
  const isIntlSecTradeSettlementWorkflow = checkFeatureFlag(featureFlags.intlSecTradeSettlementWorkflow);
  ...
  return (
    <Fragment>
      <Drawer
        ...
        anchor="left"
      >
        <Grid container direction="column" justifyContent="flex-start" alignContent="flex-start" className={style.drawer__content}>
          ...
          <Grid item className={style.drawer__items}>
            <Paper className={style.drawer__section}>
              <Grid container direction="column" justifyContent="space-between" className={style.drawer__content}>
                {kycApprovalStatus ? <NavLinkList routingConfigs={RoutingConfigs} /> : <NavLinkList routingConfigs={initialRoutingConfigs} />}
                <List>
