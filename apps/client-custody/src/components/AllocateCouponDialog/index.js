...
const computeTotalAllocatedCouponAmount = (data) => data.reduce((acc, current) => acc + parseInt(current?.couponAllocation, 10), 0);
...
const AllocateCouponDialog = ({ open, handleClose, currentlySelectedRowData }) => {
  ...
  const accountOptionsList = wethaqOwnedAccounts
    .filter(({ currencyName }) => currencyName === currentlySelectedRowData?.externalSecurity?.currencyName?.name)
    .map((item) => ({
  ...
  const handleSubmitForApproval = () => {
    const addCouponAllocation = (payload) => dispatch(couponsActionCreators.doAddCouponAllocation(payload));
    ...
    const requestPayload = {
      ...
      debitWethaqAccountId: selectedWethaqSourceAccountOption?.value?.id || currentlySelectedRowData?.debitWethaqAccountId,
    };
    ...
    addCouponAllocation({
      ...
      requestPayload,
      successCallback: () => {
        const fetchAllCouponsEvents = (payload) => dispatch(couponsActionCreators.doFetchAllCouponEvents(payload));
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <Box mb={2}>
          <CouponEventDetails couponEventData={currentlySelectedRowData} selectedWethaqSourceAccountOption={selectedWethaqSourceAccountOption} />
          <hr />
                ...
                <Grid item container md={12} justifyContent="space-between">
                  ...
                  <Grid item>
                    {selectedWethaqSourceAccountOption?.value?.accountBalance &&
                      convertNumberToIntlFormat(selectedWethaqSourceAccountOption?.value?.accountBalance, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}{' '}
                  </Grid>
                </Grid>
              </Grid>
            )}
            ...
            <Grid item md={12} container spacing={1} justifyContent="flex-end">
              <Grid item>
                <Button
                  color="primary"
                  variant="contained"
                  disabled={!totalCouponAmount || !(totalCouponAmount - computedTotalAllocatedCouponAmount === 0)}
                  type="submit"
