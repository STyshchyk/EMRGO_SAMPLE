    ...
    <Fragment>
      <Formik
        ...
        onSubmit={(values, { setSubmitting }) => {
          ...
          deassignUserAdmin(requestPayload, selectedRow?.user ? selectedRow.user.id : selectedRow.id);
        }}
      >
        {({ handleSubmit }) => (
          <Form>
            <Dialog fullWidth open={open} onClose={onClose} aria-labelledby="form-dialog-title" PaperProps={{ className: 'overflow-y-visible' }}>
              <DialogTitle id="edit-group-types-form-dialog-title">{t('onboarding:Forms.Are you sure you want to unassign user as an admin?')}</DialogTitle>
