...
import CouponPaymentScheduleTable, { generateCouponPaymentScheduleTableRowData } from '../CouponPaymentScheduleTable';
...
const EditCouponPaymentScheduleDialog = ({ currentlySelectedRowData, open, handleClose }) => {
  ...
  const couponPaymentScheduleUpdatedAt = dateFormatter(couponPaymentSchedule?.updatedAt, 'MMMM Do YYYY, HH:mm');
  ...
  // dispatchers
  const fetchAllCouponPaymentSchedules = () => dispatch(couponsActionCreators.doFetchAllCouponPaymentSchedules());
  const fetchExternalSecurities = () => dispatch(externalSecuritiesActionCreators.doFetchExternalSecuritiesList());
  const addCouponPaymentSchedule = (payload) => dispatch(couponsActionCreators.doAddCouponPaymentSchedule(payload));
  const editCouponPaymentSchedule = (payload) => dispatch(couponsActionCreators.doEditCouponPaymentSchedule(payload));

  useEffect(() => {
    const fetchCouponPaymentScheduleById = (payload) => dispatch(couponsActionCreators.doFetchCouponPaymentScheduleById(payload));
  ...
  useEffect(() => {
    ...
    const generatedTableData = couponPaymentDates.map((item) => generateCouponPaymentScheduleTableRowData(item));
  ...
  const handleUpdateCouponSchedule = () => {
    ...
    const deletedIds = originalListOfDateIds.filter((i) => !tableData.map(({ id }) => id).includes(i));
    ...
    const couponDateObject = tableData
      .filter(
        ({ couponAllocationStatus }) =>
          ...
          ![couponAllocationStatusEnum.ALLOCATED, couponAllocationStatusEnum.PENDING_APPROVAL].includes(couponAllocationStatus),
      )
  ...
  const handlePublishSchedule = () => {
    const publishCouponPaymentScheduleById = (payload) => dispatch(couponsActionCreators.doPublishCouponPaymentScheduleById(payload));
    const requestPayload = {
      isPublished: true,
    };
        ...
        <DialogContent>
          ...
          >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={12}>
                <CouponPaymentScheduleTable editable hardcodedCouponRate={currentlySelectedRowData?.profitRate ?? 0.0} tableData={tableData} setTableData={setTableData} isLoading={isLoading} />
              </Grid>
              ...
              >
                <Grid item container md={12} spacing={2} justifyContent="flex-end">
                  ...
                  {couponPaymentSchedule?.id && hasCouponScheduleDates ? (
                    <Grid item>
                      <Button variant="contained" onClick={handleUpdateCouponSchedule} color="primary">
                  ...
                  <Grid item>
                    <Button
                      ...
                      color="primary"
                      disabled={!couponPaymentSchedule?.id || isRequesting || isTableDataEmpty || couponPaymentScheduleStatus !== 'Pending'}
                    >
