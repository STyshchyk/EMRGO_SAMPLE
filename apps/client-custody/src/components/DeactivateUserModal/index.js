    ...
    <Fragment>
      <Formik
        ...
        onSubmit={(values, { setSubmitting }) => {
          ...
          deactivateEntity(requestPayload, selectedRow?.user ? selectedRow.user.id : selectedRow.id, selectedRow?.entityId);
        }}
      >
        {({ handleSubmit }) => (
          <Form>
            <Dialog fullWidth open={open} onClose={onClose} aria-labelledby="form-dialog-title" PaperProps={{ className: 'overflow-y-visible' }}>
              <DialogTitle id="edit-group-types-form-dialog-title">{t('onboarding:Forms.Deactivate User')}</DialogTitle>
              <DialogContent className="overflow-y-visible">
                <MuiPickersUtilsProvider utils={MomentUtils} locale={theme.locale.altLocale}>
                  <Grid container>
                    <Field fullWidth component={TextField} minRows={3} multiline label={t('onboarding:Forms.Reason')} name="reason" variant="filled" />
                  </Grid>
