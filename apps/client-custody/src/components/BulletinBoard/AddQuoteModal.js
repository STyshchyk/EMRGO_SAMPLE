    ...
    >
      {({ handleSubmit, values, resetForm, setFieldValue }) => (
        <form onSubmit={handleSubmit} className={cx(style.formWrapper)} noValidate>
          <Dialog open={isModalOpen} onClose={handleClose} aria-labelledby="send-onboarding-invitation" maxWidth="md" fullWidth>
            <DialogTitle id="form-dialog-title">{t('bulletin_board:Modals.Add Quote Modal.Add Quote')}</DialogTitle>
            <DialogContent>
              <Box mb={2}>
                <Fragment>
                ...
                <Box my={3} className="full-width">
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={6} container>
                      ...
                      <Grid item xs={6}>
                        <Typography align="right">{values?.issuance?.value?.currencyName?.name}</Typography>
                      </Grid>
                    </Grid>
                    <Grid item xs={6} container>
                      <Grid item xs={6}>
                    ...
                    <Grid item xs={6} container>
                      ...
                      <Grid item xs={6}>
                        <Typography align="right">{values?.issuance?.value?.maturityDate ? moment(values?.issuance?.value?.maturityDate).format('DD-MMM-YY') : ''}</Typography>
                      </Grid>
                    </Grid>
                    <Grid item xs={6} container>
                      <Grid item xs={6}>
                ...
                <Box my={1} className="full-width">
                  <Accordion expanded={expanded === 'panel1'} onChange={handleChange('panel1')} elevation={0}>
                    <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="panel1bh-content" id="panel1bh-header">
                      <Typography> {t('bulletin_board:Modals.Add Quote Modal.Add Optional Pricing Information')}</Typography>
                    </AccordionSummary>
                    <AccordionDetails>
                      <Grid container spacing={2}>
                        <Grid item xs={6}>
                          <Box className={style.indication__offer}>
                            <Typography variant="h6">{t('bulletin_board:Modals.Add Quote Modal.Indicative Offer')}</Typography>
                          </Box>
                          <Grid container spacing={2}>
                        ...
                        <Grid item xs={6}>
                          <Box className={style.indication__bid}>
                            <Typography variant="h6">{t('bulletin_board:Modals.Add Quote Modal.Indicative Bid')}</Typography>
                          </Box>
                          <Grid container spacing={2}>
                            <Grid item xs={4} container alignContent="center">
                              <Typography className={style.bold} align="left">
