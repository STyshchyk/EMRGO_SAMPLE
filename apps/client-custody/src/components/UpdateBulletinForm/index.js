...
const UpdateBulletinForm = ({ handleClose, onSubmit, open, handleFileUpload, filesUploaded, filesUploadInProgress, currentBulletin }) => {
      ...
      {({ handleSubmit, handleReset }) => (
        <form onSubmit={handleSubmit} noValidate>
          <Dialog open={open} onClose={() => handleClose(false)} aria-labelledby="new-bulletin" maxWidth="sm" fullWidth>
            <DialogTitle id="form-dialog-title">{t('bulletin:Update Bulletin Modal.Update Bulletin')}</DialogTitle>
            <DialogContent dir={locale.rtl ? 'rtl' : 'ltr'}>
              <Box mb={2}>
                <DialogContentText>{t('bulletin:Update Bulletin Modal.Bulletin Information')}</DialogContentText>
                <Box my={1} className="full-width">
                  <Field component={TextField} label={t('bulletin:Update Bulletin Modal.Bulletin Title')} name="title" variant="filled" fullWidth />
                </Box>
                <Box my={1} className="full-width">
                  <Datepicker label={t('bulletin:Update Bulletin Modal.Bulletin Date')} name="itemDate" materialLabel fullWidth />
                </Box>
                <Box>
                  <FileUploadField
                    // label="Bulletin Document"
                    isLoading={filesUploadInProgress}
                    defaultFiles={currentBulletin.documentId ? [{ file: { name: currentBulletin.documentId } }] : null}
                    name="documentId"
                    fullWidth
