...
const CorporateActionEventDetail = ({ currentlySelectedRow }) => (
  <Box display="flex">
    <Grid item xs={12} container>
      ...
      <Grid item xs={12} md={6}>
        <Data label="Ex Date" value={dateFormatter(currentlySelectedRow?.exDate, DEFAULT_DATE_FORMAT)} />
        <Data label="Record Date" value={dateFormatter(currentlySelectedRow?.recordDate, DEFAULT_DATE_FORMAT)} />
        <Data label="Payment Date" value={dateFormatter(currentlySelectedRow?.paymentDate, DEFAULT_DATE_FORMAT)} />
      </Grid>
    </Grid>
  </Box>
);
