...
import CorporateActionEventsResponses, { generateTableRowData } from './CorporateActionEventResponses';
...
const ViewCorporateActionEventDialog = ({ open, handleClose, currentlySelectedRowData, setCurrentlySelectedRowData, currentEntityType, title, showResponses, isReadOnly }) => {
  ...
  useEffect(() => {
    if (Array.isArray(validInvestors) && validInvestors.length > 0) {
      const updatedTableData = validInvestors?.map((inv) => {
        const foundResponse = allResponses?.find((res) => inv.entityId === res?.entityGroup?.entityId);
  ...
  useEffect(() => {
    const fetchCorporateActionEvent = (payload) => dispatch(CAEventsActionCreators.doFetchCAEventbyId(payload));

    const resetCAEventState = () => dispatch(CAEventsActionCreators.doResetCAEventState());

    if (currentEntityType === 'EMRGO_SERVICES' && currentlySelectedRowData?.mandatoryOrVoluntary === 'V') {
      fetchCorporateActionEvent({ requestPayload: { corporateActionEventId: currentlySelectedRowData?.id } });
    }

    return () => resetCAEventState();
  }, [currentlySelectedRowData?.id, dispatch, currentEntityType, currentlySelectedRowData?.mandatoryOrVoluntary]);
            ...
            <DialogContent>
              ...
              {!showResponses && (
                <>
                  ...
                  <Section label="Additional Information" value={currentlySelectedRowData?.additionalInfo} />

                  <DataGridRow label={'Voluntary/Mandatory'} value={currentlySelectedRowData?.mandatoryOrVoluntary} />
                </>
              )}

              <DataGridRow label={'Client Response Deadline'} value={dateFormatter(currentlySelectedRowData?.responseDeadline, DEFAULT_DATE_FORMAT)} />

              {showResponses && <CorporateActionEventsResponses tableData={tableData} isFetchingEvent={isFetchingEvent} />}
              ...
              {isUserInvestor && !isReadOnly && (
                <InlineFormField label="Response">
                  <Field fullWidth component={TextField} name="clientResponse" value={values.clientResponse} variant="filled" type="text" multiline rows={4} />
                </InlineFormField>
              )}
              <DialogActions>
