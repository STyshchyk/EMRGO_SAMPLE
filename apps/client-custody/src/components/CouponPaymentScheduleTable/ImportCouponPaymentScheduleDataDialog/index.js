...
const ImportCouponPaymentScheduleDataDialog = ({ open, handleClose, setTableData }) => {
  const onDrop = useCallback((acceptedFiles) => {
    ...
    const isFileTypeXLSX = fileType === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
  ...
  const { getRootProps, getInputProps, isDragActive, isFocused, isDragAccept, isDragReject } = useDropzone({ onDrop, maxFiles: 1, accept: ALLOWED_FILE_TYPES });
