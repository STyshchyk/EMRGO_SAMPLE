...
const CouponPaymentScheduleTable = ({ hardcodedCouponRate, tableData, setTableData, isLoading, editable }) => {
  ...
  const handleCSVExportClick = (data) => {
    ...
    const listOfRows = transformed.map(({ date, notionalAmount, couponRate }) => [[date], [notionalAmount], [couponRate]]);
  ...
  const disableDay = (date) => {
    const listOfNormalizedDateISOStrings = tableData.map((item) => normalizeDateObject(new Date(item.calenderDate)).toISOString());
    ...
    return givenDateObject.toISOString() === todaysDateObject.toISOString() || listOfNormalizedDateISOStrings.includes(givenDateObject.toISOString());
  };
      ...
      <MaterialTable
            ...
            editComponent: (props) => (
              <MuiPickersUtilsProvider utils={MomentUtils}>
                <DatePicker
                  ...
                  className={'MuiInputBase-root MuiInput-root MuiInput-underline Mui-error MuiInputBase-fullWidth MuiInput-fullWidth MuiInputBase-formControl MuiInput-formControl'}
                />
                {props.error && <div className="MuiFormHelperText-root Mui-error">{props.helperText}</div>}
        ...
        components={{
          Action: (props) => {
            ...
            if (['Save', 'Cancel'].includes(props.action?.tooltip)) return <MTableAction {...props} />;

            const disabled = !isAfterToday || [couponAllocationStatusEnum.PENDING_APPROVAL, couponAllocationStatusEnum.ALLOCATED].includes(couponAllocationStatus);
          ...
          Toolbar: (props) => {
            ...
            const disableImportCSVDataButton = data.some(({ couponAllocationStatus }) =>
              [couponAllocationStatusEnum.PENDING_APPROVAL, couponAllocationStatusEnum.ALLOCATED].includes(couponAllocationStatus),
            );
            ...
            return (
              <Grid container justifyContent="flex-end" alignItems="center" spacing={1}>
                ...
                {editable && (
                  <Grid item>
                    <Button size="small" variant="contained" color="secondary" onClick={handleAddClick}>
                      <strong>ADD +</strong>
                    </Button>
                  </Grid>
                )}
