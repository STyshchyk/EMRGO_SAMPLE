...
const FXTransactionActionModal = ({ open, action, onClose, selectedRow, currentlySelectedDateRange }) => {
        ...
        {({ handleSubmit }) => (
          <Form>
            <Dialog fullWidth open={open} onClose={onClose} aria-labelledby="form-dialog-title" PaperProps={{ className: 'overflow-y-visible' }}>
              <DialogTitle>{isApprove ? t('fx_transactions:Fx Action Modal.Approve Transaction') : t('fx_transactions:Fx Action Modal.Cancel Transaction')}?</DialogTitle>
              <DialogContent>
                {isApprove
                  ? t('fx_transactions:Fx Action Modal.Are you sure you want to approve this transaction? This action is non reversible')
                  : t('fx_transactions:Fx Action Modal.Are you sure you want to cancel this transaction? This action is non reversible')}
              </DialogContent>
              <DialogActions>
                <Grid container justifyContent="flex-end" className="mx-4 mb-4">
                  ...
                  <Grid item xs={12} lg={5}>
                    <Button type="submit" variant="contained" color="primary" fullWidth onClick={handleSubmit}>
                      {isApprove ? t('fx_transactions:Fx Action Modal.Approve Transaction') : t('fx_transactions:Fx Action Modal.Cancel Transaction')}
                    </Button>
                  </Grid>
                </Grid>
              </DialogActions>
