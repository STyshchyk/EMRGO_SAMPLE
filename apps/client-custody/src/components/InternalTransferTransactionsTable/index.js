...
const TableActionMenu = ({ handleCloseMenu, actions, anchorEl }) => (
  <Fragment>
    {Boolean(anchorEl) && (
      <Menu data-testid="internal-transfer-transactions-table-menu-list" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleCloseMenu} disableScrollLock>
        {actions
...
const generateClientDropdownOptionsList = (tableData) => {
  const collectionOfClientNames = tableData
    .map(({ sourceAccountOwner, destinationAccountOwner }) => [sourceAccountOwner, destinationAccountOwner])
    .flat()
    .sort();
...
const InternalTransferTransactionsTable = ({ anchorEl, handleCloseMenu, data, actions, setCurrentlySelectedRowData, setAnchorEl, isLoading }) => {
    ...
    <Fragment>
      ...
      >
        <FilterProvider tableKey="internal_transfer_transactions">
          ...
          >
            <TableFiltersWrapper tableRef={tableRef} data={data} columns={tableColumns} open={true} hideExportButtons>
              <Grid container spacing={2}>
                ...
                <Fragment>
                  <Grid item xs={12} md={6} lg={3}>
                    <DropdownFilter name="client" label="Client" options={generateClientDropdownOptionsList(data)} />
                  </Grid>
                </Fragment>
                <Grid item xs={12} md={6} lg={3}>
                  <DropdownFilter
            ...
            {({ filters, filterColumns }) => {
              const filteredData = data
                ...
                .filter((row) => {
                  if (filters?.client) {
                    ...
                    return row.sourceAccountOwner === clientFilterValue || row.destinationAccountOwner === clientFilterValue;
                  }
                  return true;
                })
                .filter((row) => {
              ...
              return (
                <div data-testid="security-trades-table">
                  ...
                  <ReportingTablePDFExporter ref={ref} title="Internal Transfer Transactions" pageSize="A2">
                    <ExportTableContent
