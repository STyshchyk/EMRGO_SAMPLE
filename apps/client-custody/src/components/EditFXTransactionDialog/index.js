...
import { BANK_RATE_DP, MARKUP_RATE_DP, CLIENT_RATE_DP, BANK_AMOUNT_DP, MARKUP_AMOUNT_DP, CLIENT_AMOUNT_DP } from '../../constants/currency/availableCurrencies';
    ...
    <>
      ...
      {values.overrideFlag && (
        <FormHelperText id="component-helper-text">
          ...
          <ButtonBase color="primary" style={{ textDecoration: 'underline' }} onClick={() => resetValue()}>
            here
          </ButtonBase>{' '}
          to reset.
        </FormHelperText>
...
const EditFXTransactionDialog = ({ open, handleClose, selectedRow, currentlySelectedDateRange }) => {
  ...
  const entityOptionsList = entitiesList?.map((entity) => ({ label: entity.corporateEntityName, value: entity.id }));
  ...
  useEffect(() => {
    const fetchAccounts = (payload) => dispatch(billingAndPaymentsActionCreators.doFetchAccounts(payload));
  ...
  useEffect(() => {
    const fetchFormOptions = (payload) => dispatch(billingAndPaymentsActionCreators.doFetchDropdownValues(payload));
    fetchFormOptions({
      options: ['currency'],
    });
  }, [dispatch]);
  ...
  const generateAccountOptionsList = (entity, currency) => {
    ...
    if (entity) {
      ...
      filteredOptions = accounts.map((acc) => ({ label: acc?.accountNo, value: acc?.accountNo, id: acc?.id }));
  ...
  const selectedEntityOption = entityOptionsList.find((entityOption) => entityOption.label === selectedRow?.entity);
  const selectedFromCurrencyOption = currencyOptionsList.find((currencyOption) => currencyOption.label === selectedRow?.fromCurrency);
  const selectedToCurrencyOption = currencyOptionsList.find((currencyOption) => currencyOption.label === selectedRow?.toCurrency);
  const selectedFromAccountOptionsList = generateAccountOptionsList(selectedEntityOption, selectedFromCurrencyOption);
  const selectedToAccountOptionsList = generateAccountOptionsList(selectedEntityOption, selectedToCurrencyOption);
  const selectedFromAccountOption = selectedFromAccountOptionsList.find((fromAccountOption) => fromAccountOption.label === selectedRow?.fromAccount);
  const selectedToAccountOption = selectedToAccountOptionsList.find((toAccountOption) => toAccountOption.label === selectedRow?.toAccount);
  const selectedExchangeRate = Number(selectedRow.fromAmount) / Number(selectedRow.toAmount);

  return (
    <Dialog fullWidth open={open} onClose={handleClose} aria-labelledby="form-dialog-title" scroll="body">
      <Formik
        initialValues={{
      ...
      >
        {({ values, handleSubmit, setFieldValue, setValues, setFieldTouched, setTouched, touched, handleBlur }) => {
          const toCurrencyOptionsList = generateToCurrencyOptionsList(values?.fromCurrency?.value);
          const fromAccountOptionsList = generateAccountOptionsList(values?.entity, values?.fromCurrency);
          const toAccountOptionsList = generateAccountOptionsList(values?.entity, values?.toCurrency);
          ...
          const resetCalculationFields = () => {
            ...
            setTouched({ ...touched, bankRate: false, markupRate: false, clientRate: false, bankAmount: false, markupAmount: false, clientAmount: false });
          };

          return (
            <form onSubmit={handleSubmit}>
              <DialogTitle id="form-dialog-title"> {t('fx_transactions:Fx Modal.Edit Fx Transaction')}</DialogTitle>
                  ...
                  <Grid container>
                    <Grid container className="mt-4">
                      <Grid item xs={12} md={6} lg={6} alignContent="flex-start">
                        <Typography className="mt-4">{t('fx_transactions:Fx Table.Entity')}</Typography>
                      </Grid>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        ...
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="fromCurrency" />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        <FormControl className="w-full">
                          <DependentAccountField name="fromAccount" optionsList={fromAccountOptionsList} value={values?.fromAccount} currency={values?.fromCurrency} />
                        </FormControl>
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="fromAccount" />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container className="mt-4" justifyContent="flex-end">
                      <Grid item xs={5} className="px-1">
                        <Button
                          ...
                          color="primary"
                          onClick={() => {
                            runFxCalculations({ values, setFieldValue, touched, incompleteText: t('fx_transactions:Fx Modal.Insufficient information given') });
                          }}
                        >
                      ...
                      <Grid item container xs={1} className="px-1" alignContent="center">
                        <Tooltip title={t('fx_transactions:Fx Modal.Buttons.Reset')}>
                          <IconButton size="small" aria-label="reset" color="primary" onClick={() => resetCalculationFields()}>
                            <RefreshIcon />
                          </IconButton>
                        </Tooltip>
                      </Grid>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        ...
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="toCurrency" />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container className="mt-4">
                      ...
                      <Grid item xs={12} md={6} lg={6} alignContent="center" className="px-1">
                        <FormControl className="w-full">
                          <DependentAccountField name="toAccount" optionsList={toAccountOptionsList} value={values?.toAccount} currency={values?.toCurrency} />
                        </FormControl>
                        <ErrorMessage component={Typography} variant="caption" color="error" className="ml-4" name="toAccount" />
                      </Grid>
                    </Grid>
