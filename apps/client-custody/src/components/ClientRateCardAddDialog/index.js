...
const ClientRateCardAddDialog = ({ open, handleClose }) => {
  ...
  const entitiesDropdown = isWethaqUser ? filteredInvestors.map((entity) => ({ value: entity.id, label: entity.corporateEntityName, groups: entity.groups })) : null;
        ...
        <MuiPickersUtilsProvider utils={MomentUtils} locale={theme.locale.altLocale}>
          ...
          >
            {({ values, handleSubmit, setFieldValue }) => {
              const filteredAccounts = sourceAccounts?.filter((account) => account.value.entityGroupId === values.entityGroupId).filter((account) => account.value.currencyId === USDCurrency.id);
              return (
                <form onSubmit={handleSubmit}>
                  <DialogTitle id="form-dialog-title"> {t('Client Rate Card.Add Dialog.Add Client Rate Card')}</DialogTitle>
                      ...
                      <Grid container spacing={2}>
                        <Grid item xs={12} md={12} lg={6} container>
                          <Grid item xs={6} alignContent="flex-start">
                            <Typography className="mt-4">{t('Client Rate Card.Add Dialog.Client')}</Typography>
                          </Grid>
                            ...
                            <FormControl className="w-full">
                              <Select
                                ...
                                onChange={(selectedEntity) => {
                                  ...
                                  const investorGroup = selectedEntity.groups.find((group) => group.entityType === 'INVESTOR');
                                  setFieldValue('entityId', selectedEntity.value);
                                  setFieldValue('entityGroupId', investorGroup.id);
                                }}
                              />
                            </FormControl>
                            <ErrorMessage component={Typography} variant="caption" color="error" className="ml-2" name="entityId" />
                          </Grid>
                        </Grid>
                        <Grid item xs={12} md={12} lg={6} container>
                          <Grid item xs={6} alignContent="flex-start">
                            <Typography className="mt-4">{t('Client Rate Card.Add Dialog.Billing Account')}</Typography>
                          </Grid>
                          <Grid item xs={6} alignContent="center" className="px-1">
                            <FormControl className="w-full">
                              <Select
                            ...
                            <ErrorMessage component={Typography} variant="caption" color="error" className="ml-2" name="billingAccount" />
                          </Grid>
                        </Grid>
                        <Grid item xs={12} md={12} lg={6} container>
                          <Grid item xs={6} alignContent="flex-start">
                            <Typography className="mt-4">{t('Client Rate Card.Add Dialog.Billing Period')}</Typography>
                          </Grid>
                          <Grid item xs={6} alignContent="center" className="px-1">
                            <FormControl className="w-full">
                              <Select
                            ...
                            <ErrorMessage component={Typography} variant="caption" color="error" className="ml-2" name="billingPeriod" />
                          </Grid>
                        </Grid>
                        <Grid item xs={12} md={12} lg={6} container>
                          <Grid item xs={6} alignContent="flex-start">
                            <Typography className="mt-4">{t('Client Rate Card.Add Dialog.Minimum Charge (USD)')}</Typography>
                          </Grid>
                          <Grid item xs={6} alignContent="center" className="px-1">
                            <Field fullWidth component={TextField} name="minimumCharge" variant="filled" />
                          </Grid>
                        </Grid>
                        <Grid item xs={12} md={12} lg={6} container>
                          <Grid item xs={12} md={6} lg={6}>
                            <Typography className="mt-4">{t('Client Rate Card.Add Dialog.Start Date')} </Typography>
                          </Grid>
                        ...
                        <Grid item xs={12} md={12} lg={6} container>
                          <Grid item xs={12} md={6} lg={6}>
                            <Typography className="mt-4">{t('Client Rate Card.Add Dialog.End Date')} </Typography>
                          </Grid>
                  ...
                  <DialogActions>
                    <Grid container justifyContent="flex-end" className="w-full">
                      <Grid item lg={6}>
                        <FormControlLabel control={<Checkbox checked={addMore} onChange={handleAddMoreChange} name="checkedA" />} label={t('Client Rate Card.Add Dialog.Add more Rate Cards?')} />
                      </Grid>
                      <Grid item lg={3}>
                        <Button
                          fullWidth
