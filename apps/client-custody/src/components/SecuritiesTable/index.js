...
const TableActionMenu = ({ handleCloseMenu, actions, anchorEl }) => (
  <Fragment>
    {Boolean(anchorEl) && (
      <Menu data-testid="securities-table-menu-list" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleCloseMenu} disableScrollLock>
        {actions.map((action) => (
...
const generateSecuritiesTableRowData = (i) => {
  ...
  return {
    ...
    id: i.id,
    isin: i.isin ?? '--',
    issuerEntityCorporateName: i.projectTeam?.find(({ group }) => group?.entityType === 'ISSUER')?.group?.entity?.corporateEntityName,
...
const SecuritiesTable = ({ anchorEl, handleCloseMenu, data, actions, setCurrentlySelectedRowData, showAllFilters, setAnchorEl, entityUserType }) => {
  ...
  const dedupedIssuerEntityNames = Array.from(new Set(data.map((item) => item.issuerEntityCorporateName))).sort();
  ...
  const headCells = [
    ...
    },
    {
      ...
      exportConfig: { render: (rowData) => dateFormatter(rowData?.dateSubmitted, DEFAULT_DATE_FORMAT), width: 8 },
    },
    {
      id: 'settlementDate',
      ...
      exportConfig: { render: (rowData) => dateFormatter(rowData?.settlementDate, DEFAULT_DATE_FORMAT), width: 8 },
    },
    {
      id: 'valueDate',
      title: 'Value Date',
      ...
      exportConfig: { render: (rowData) => dateFormatter(rowData?.issueDate, DEFAULT_DATE_FORMAT), width: 8 },
    },
    {
      id: 'wsn',
      title: 'WSN',
  ...
  return (
    <Fragment>
      <FilterProvider tableKey="security_registration_table">
        ...
        >
          <TableFiltersWrapper tableRef={tableRef} data={data} columns={headCells} open={false} hideExportButtons>
            <Grid container spacing={2}>
              <Grid item xs={12} md={12} lg={6}>
                <DateRangePicker name="transactionDateRange" label={t('miscellaneous:Filters.Date')} defaultFilter="quarter" />
              </Grid>
            </Grid>
            <Grid container spacing={2}>
              {showAllFilters && (
        ...
        <FilterConsumer>
          {({ filters, filterColumns }) => {
            const filteredData = data
              .filter((row) => {
                // Date range Filter
                if (filters?.transactionDateRange?.value?.startDate && filters?.transactionDateRange?.value?.endDate) {
