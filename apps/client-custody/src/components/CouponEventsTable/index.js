...
const TableActionMenu = ({ handleCloseMenu, actions, anchorEl }) => (
  <Fragment>
    {Boolean(anchorEl) && (
      <Menu data-testid="coupon-events-table-menu-list" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleCloseMenu} disableScrollLock>
        {actions.map((action) => (
...
const CouponEventsTable = ({ anchorEl, handleCloseMenu, actions, data, setCurrentlySelectedRowData, setAnchorEl, isLoading }) => {
  ...
  const tableColumns = [
    {
      ...
      exportConfig: { render: (rowData) => dateFormatter(rowData?.couponDate, DEFAULT_DATE_FORMAT) },
    },
    {
      id: 'security',
      title: 'Security',
          ...
          <TableFiltersWrapper tableRef={tableRef} data={data} columns={tableColumns} open={true} hideExportButtons>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={6} lg={3}>
                <DropdownFilter name="security" label="Security" options={securityFilterOptionsList} />
              </Grid>
              <Grid item xs={12} md={6} lg={3}>
                <DropdownFilter name="status" label="Status" options={couponAllocStatusFilterOptionsList} />
              </Grid>
            </Grid>
          </TableFiltersWrapper>
          <FilterConsumer>
