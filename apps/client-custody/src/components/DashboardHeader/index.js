...
const DashboardHeader = ({ open, handleDrawerToggle }) => {
  ...
  const drawerToggleIcon = () => {
    let toggleIcon = '';
    if (locale.rtl) {
      toggleIcon = open ? <ChevronRightIcon fontSize="inherit" /> : <ChevronLeftIcon fontSize="inherit" />;
    } else {
      toggleIcon = open ? <ChevronLeftIcon fontSize="inherit" /> : <ChevronRightIcon fontSize="inherit" />;
    }
    return toggleIcon;
  };
      ...
      <AppBar position="fixed" className={cx(style.appbar)}>
        <Toolbar className={style.toolbar} datatest-id="dashboard-header-menu">
          ...
          <Grid item container alignContent="center" justifyContent="flex-end">
            <Typography variant="body1" display="inline" color="textSecondary" className={style.clock}>
        ...
        <Box pb={3} className={style.menu__wrapper}>
          <div className={style.menu__header}>
            <Typography variant="subtitle2" align="center" className={style.menu__current__group}>
              {locale.rtl
                ? `(${changeCase.titleCase(t(`EntityGroupType.${currentEntityType}`))}) ${t(`EntityGroupName.${currentEntityGroup?.name}`)}`
                : `${t(`EntityGroupName.${currentEntityGroup?.name}`)} (${changeCase.titleCase(t(`EntityGroupType.${currentEntityType}`))})`}
            </Typography>
          </div>
          <Box px={2} pt={2}>
            <Grid container justifyContent="center" alignContent="center">
              <Avatar className={style.menu__main_avatar} alt={currentEntityGroup?.name} src="/broken-image.jpg">
