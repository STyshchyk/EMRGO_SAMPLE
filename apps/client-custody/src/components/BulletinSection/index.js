...
const BulletinSection = ({ data, setCurrentBulletin, setIsDocumentModalOpen, setDeleteBulletinConfirmationModal, fetchBulletinDocument, setShowUpdateBulletinModal }) => {
  const { t } = useTranslation(['translation', 'bulletin']);
  const { theme } = useTheme();
  const { locale } = theme;
  const mtableLocalization = useMaterialTableLocalization();
  const currentEntityGroup = useSelector(authSelectors.selectCurrentEntityGroup);
  const hasEditBulletinEdit = currentEntityGroup?.accessControls.some((acl) => acl?.key === accessControlsList.KYC.manage.key);
  ...
  if (hasEditBulletinEdit) {
    columns.push({
      ...
      field: 'title',
      width: '70%',
      cellStyle: { textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden', maxWidth: 100, ...cellStyle },
    });
    columns.push({ title: t('bulletin:Author'), field: 'user.firstName', cellStyle: { ...cellStyle } });
  } else {
    columns.push({
      title: t('bulletin:Title'),
      field: 'title',
  ...
  const getTranslatedType = (item) => (locale.altLocale === 'ar-sa' ? item.type.nameAr : item.type.name);

  return (
    <Box py={2} className={cx(hasEditBulletinEdit ? style.container__full : style.container__half)} data-testid="bulletin-table">
      <MaterialTable
