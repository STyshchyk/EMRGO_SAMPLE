...
import { fetchProfileSettingsSilver, postProfileSettings } from "./profile";

export function useDarkModeCustomSilver(): [boolean, () => void, () => void, () => void, boolean] {
  ...
  const {
    ...
    cacheTime: Infinity,
    queryFn: () => fetchProfileSettingsSilver({ keys: ["dark-mode-settings"] }),
    onSuccess: (value) => {
      if (value) enableDark();
      else disableDark();
    },
